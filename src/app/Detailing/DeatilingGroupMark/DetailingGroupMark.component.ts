...
// import math from 'mathjs'; tanmay
...
export class DetailingGroupMarkComponent implements OnInit {
  ...
  pushElement: any[] = [];
  // storedObjectData: any //duplicate
  // MeshData: any//duplicate
  //elementDetails:any[]=[];
  ...
  strIsReadOnly: string = 'NO';
  disableparamterset: boolean = false;
  ParentStructureMarkId: number = 0;
  CoreCageProductList: any[] = [];
  SelectedProductCodeId: any = 0;
  cagSelection:any[]=[
    {'CageId':1,'Cage_type':'Odd'},
    {'CageId':2,'Cage_type':'Normal'},
  ]
  ...
  constructor(
    ...
    public beamDetailingService: BeamDetailingService,
  ...
  constructor_Data()
  {
    ...
    this.templateColumns = [
      ...
      {
        id: 'ElementMark',
        name: 'Prefix',//'Element Mark/ Page No\n结构名/页',//Prefix
        field: 'ElementMark',
    ...
    // let s = this.router.routeReuseStrategy;
    ...
    ///detailing/DetailingGroupMark/BorePile
    this.IntGroupMarkId = this.MeshData.INTGROUPMARKID;
    if (this.structureElementId == 8) {
      this.router.navigate(['/detailing/DetailingGroupMark/BorePile']);
    } else if (this.structureElementId == 5) {
      this.router.navigate(['/detailing/DetailingGroupMark/Drain']);
    }
    else if (this.MeshData.VCHPRODUCTTYPE === 'CAR') {
      this.router.navigate(['/detailing/DetailingGroupMark/Carpet']);
    }
    else if (this.MeshData.VCHPRODUCTTYPE === 'PRC') {
      this.LoadGroupMarkIDPRC(this.MeshData.INTGROUPMARKID);
      this.LoadSPAMaterialForPRC();
    } else if (this.MeshData.VCHPRODUCTTYPE === 'CORE') {
    ...
    this.parameterList.push({
      value: this.Queryparameterset,
      text: this.Queryparameterset,
    });
  }

  ngOnInit() {
    //CAB Details entyr start
  ...
  loadInitialData()
  {
    ...
    this.isEdit_Groupmarking = this.MeshData.isEdit;
    console.log("this.MeshData=>",this.MeshData,this.structureElementId);
    this.loadParameterSetDropdown();
    this.reload_AddnParLimit();
    if (
      this.MeshData.VCHPRODUCTTYPE === 'PRC' ||
      this.MeshData.VCHPRODUCTTYPE === 'CORE'
    ...
    //this.getShapeCodeList('0001101170', '0000113012', 'N-Splice');
    ...
    if (
      ...
      this.structureElementId == 6 ||
      this.structureElementId == 58 ||
      this.structureElementId == 69 ||
      this.structureElementId == 13
    ) {
      this.SlabHeading = this.structElement;
  ...
  loadParameterSetDropdown() {
    this.loading = true;
    this.detailingService
      .Get_ParameterSet_dropdown(this.projectId, 7)
      .subscribe({
        next: (response) => {
          console.log('parameteer set vanita');
          console.log(response);
          console.log(this.MeshData.INTPARAMETESET);
          this.parameterList = response;
          this.ParameterSet = this.parameterList.find(
            (x) => x.INTPARAMETESET === this.MeshData.INTPARAMETESET
          );
        },
        error: (e) => {
          //console.log("error",e);
        ...
        complete: () => {
          ...
          {
            console.log("before=>",this.MeshData,this.structureElementId);
  ...
  LoadSPAMaterialForPRC() {
    //GetSAPMaterialforPRC
    this.columnDetailingService
      .GetSAPMaterialforPRC(this.structureElementId)
      .subscribe({
        next: (response) => {
          //console.log(response);
          this.SAPMaterialforPRC = response;
          //console.log("PRC Sap Material");
          //console.log(this.SAPMaterialforPRC)  ;
        },
        error: (e) => {
          //console.log("error",e);
        },
  ...
  changeSAP_PRC(sapmaterial: any) { }
  changeCoreCage_Product(productcode: any) { }
  SavePRCGroupMarkDetails() {
    debugger;
    if(this.MeshData.VCHPRODUCTTYPE ==='CORE')
    {
      if(this.pushGroupMark.ProductCodeId==0  || this.pushGroupMark.ProductCodeId==undefined)
      {
        this.tosterService.warning("Please select ProductCode");
        return ;
      }
    }
    else{
      if(this.pushGroupMark.SapMaterialcode==0  || this.pushGroupMark.SapMaterialcode==undefined)
      {
        this.tosterService.warning("Please select ProductCode");
        return ;
      }
    ...
    const PRCGroupMarkObj: Add_GroupMarkPRC = {
      GroupMarkId: Number(this.MeshData.INTGROUPMARKID),
      GroupMarkingName: this.MeshData.VCHGROUPMARKINGNAME,
      GroupRevisionNumber: Number(this.MeshData.TNTGROUPREVNO),
      StructureElementTypeId: Number(this.MeshData.INTSTRUCTUREELEMENTTYPEID),
      SeDetailId: Number(this.MeshData.INTSEDETAILINGID),
      SitProductTypeId: Number(this.MeshData.SITPRODUCTTYPEID),
      IsCABDE: Number(this.MeshData.ISCABDE),
      SiderForCode: this.MeshData.SIDEFORCODE,
      SAPMaterial:
        this.MeshData.VCHPRODUCTTYPE == 'PRC'
          ? Number(this.pushGroupMark.SapMaterialcode)
          : Number(this.pushGroupMark.ProductCodeId),
      ProjectId: Number(this.MeshData.INTPROJECTID),
      DefaultWidth: Number(this.pushGroupMark.Width),
      DefaultDepth: Number(this.pushGroupMark.Depth),
      DefaultLength: Number(this.pushGroupMark.Length),
      AssemblyInd: Number(this.pushGroupMark.AssemblyInd),
      CageSelector_Id: Number(this.pushGroupMark.SelectionId)
    };
    console.log(PRCGroupMarkObj);
    this.columnDetailingService
      .SavePRC_GroupMarkDetails(PRCGroupMarkObj, this.MeshData.INTSEDETAILINGID)
      .subscribe({
        next: (response) => {

          console.log(response);
          this.ParentStructureMarkId = response[0].ParentStructureMarkId;
          localStorage.setItem(
            'ParentStructureMarkId',
            this.ParentStructureMarkId.toString()
          );
          // this.transferObject['ParentStructureMarkId'] =
          //   response[0].ParentStructureMarkId;
          // localStorage.setItem(
          //   'ParentStructureMarkId',
          //   JSON.stringify(this.transferObject)
          // );
        },
        error: (e) => {
          //console.log("error",e);
          this.tosterService.error("Value not saved ")

        },
        complete: () => {
          if(this.ParentStructureMarkId==0)
          {
            this.SavePRCGroupMarkDetails()
          }
          else{
            this.tosterService.success("Value saved succesfully")
            this.transferObject['ParentStructureMarkId'] =
              this.ParentStructureMarkId;
              localStorage.setItem(
                'ParentStructureMarkId',
                this.ParentStructureMarkId.toString()
              );
  ...
  LoadGroupMarkIDPRC(INTGROUPMARKID: any) {
    debugger;
    this.columnDetailingService
      .GetPRCSAPHeaderValuesByGroupMarkId(INTGROUPMARKID)
      .subscribe({
        next: (response) => {
          console.log('SAP Material by me Tanny' ,response);

          this.SAPMaterialdata = response;
          //console.log(this.SAPMaterialdata[0].ParentStructureMarkId);

          if (localStorage.getItem('ParentStructureMarkId') !== null) {
            localStorage.removeItem('ParentStructureMarkId');
          }
          localStorage.setItem(
            'ParentStructureMarkId',
            this.SAPMaterialdata[0].ParentStructureMarkId
          );

          // this.pushGroupMark = {
          //   ProductCodeId: this.SelectedProductCodeId,
          //   SapMaterialcode: this.SAPMaterialdata[0].SAPMaterial.MaterialCodeID,
          //   Width: this.SAPMaterialdata[0].DefaultWidth,
          //   Depth: this.SAPMaterialdata[0].DefaultDepth,
          //   Length: this.SAPMaterialdata[0].DefaultLength,
          //   AssemblyInd: this.SAPMaterialdata[0].AssemblyInd,
          // };

          //console.log(this.pushGroupMark);
        },
        error: (e) => {
          //console.log("error",e);
        },
        complete: () => {
          console.log('SAP Material master');
          console.log(this.SAPMaterialdata);
          if (this.SAPMaterialdata.length == 0) {
            if (this.structureElementId == 2) {
              this.pushGroupMark = {
                ProductCodeId: this.CoreCageProductList[0].ProductCodeId,
                SapMaterialcode: this.SAPMaterialforPRC[0].MaterialCodeID,
                Width: 200,
                Depth: 400,
                Length: 300,
                AssemblyInd: true,
                SelectionId:this.SAPMaterialdata?.[0]?.SelectionId ?? 2
              };

            } else if (this.structureElementId == 13) {
              this.pushGroupMark = {
                ProductCodeId: this.SelectedProductCodeId,
                SapMaterialcode: this.SAPMaterialforPRC[0].MaterialCodeID,
                // SapMaterialcode: this.SAPMaterialdata[0].SAPMaterial.MaterialCodeID,
            ...
            else if (this.structureElementId == 6) {
              this.pushGroupMark = {
                ProductCodeId: this.SelectedProductCodeId,
                SapMaterialcode: this.SAPMaterialforPRC[0].MaterialCodeID,
                // SapMaterialcode: this.SAPMaterialdata[0].SAPMaterial.MaterialCodeID,
          ...
          if(this.MeshData.VCHPRODUCTTYPE === 'CORE' && this.isEdit_Groupmarking===false ){
            this.SelectedProductCodeId =undefined;
            this.pushGroupMark.ProductCodeId = undefined;
          }
        },
      });
  }
  //PopulateCoreCageProductCode
  SelectedCoreCageProductCode(INTGROUPMARKID: any) {

    this.columnDetailingService
      .CoreCageSelectdProductCode(INTGROUPMARKID)
      .subscribe({
        next: (response) => {
          if(this.isEdit_Groupmarking)
          {
            this.SelectedProductCodeId = response[0].ProductCodeId;
            this.pushGroupMark.ProductCodeId = this.SelectedProductCodeId;
          }
          else{
            this.SelectedProductCodeId =undefined;
            this.pushGroupMark.ProductCodeId = undefined;
  ...
  LoadProductCodeCoreCage() {
    this.columnDetailingService.PopulateCoreCageProductCode().subscribe({
      next: (response) => {

        this.CoreCageProductList = response;
        console.log('Core Cage Product');
        console.log(this.CoreCageProductList);
      },
      error: (e) => {
        //console.log("error",e);
      },
 ...
 async grid_onSelectedRowsChanged(e: any, args: any) {
    ...
    if (args.rows.length > 0) {
      if (args.rows[0] != this.barRowIndex[this.gridIndex]) {
       await this.Insert_BarMark(this.gridIndex, this.barRowIndex[this.gridIndex]); //Tab#, Row#
  ...
  async getShapeImagesByCat_CAB(
    ...
    try {
      ...
      CouplerType = 'N-Splice'; // added by ajit
      const data = await this.orderService
  ...
  InsertColumn(pPara: any, pGridID: any) {
    ...
    console.log('this.templateColumns', columns);
    console.log('this.Length', columns.length);
  ...
  LoadBeamParameterSet(projectID: any) {
    //
    debugger;
    console.log("after LoadBeamParameterSet=>",this.MeshData,this.structureElementId);
    let productTypeID = 7
    this.loading = true;
    this.beamDetailingService.BeamParameterSetbyProjIdProdType(projectID, productTypeID).subscribe({
      next: (response) => {
        //console.log(response);
        debugger;
        this.ParameterSetList_forView = response;
        console.log("next LoadBeamParameterSet=>",this.MeshData,this.structureElementId);
      ...
      complete: () => {
        ...
        this.loading = false;
        this.selected_ParameterSet = this.ParameterSetList_forView.find((x: any) => x.ParameterSetValue === this.ParameterSet.INTPARAMETESET);
        if (this.selected_ParameterSet !== undefined) {
          this.Description = this.selected_ParameterSet.Description;
  ...
  Report_Click() {
    ...
    let ProducttypeId = this.MeshData.SITPRODUCTTYPEID
