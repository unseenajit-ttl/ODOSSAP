...
import { Insert_BPC_Structuremarking_Temp } from 'src/app/Model/Insert_BPCStructuremarking';
import { MainBarPatternComponent } from './main-bar-pattern/main-bar-pattern.component';
import { ElevationPopupComponent } from './elevation-popup/elevation-popup.component';
import { AdditionalSpiralPopupComponent } from './additional-spiral-popup/additional-spiral-popup.component';
import { CageNotesPopupComponent } from './cage-notes-popup/cage-notes-popup.component';
import { CircularRingPopupComponent } from './circular-ring-popup/circular-ring-popup.component';
import { StiffnerRingPopupComponent } from './stiffner-ring-popup/stiffner-ring-popup.component';
import { BoredPilePoupModal } from 'src/app/Model/bore_pile_detailing_harcoded_value';
import { round } from 'mathjs';
import * as math from 'mathjs';
import { AngularGridInstance, Column, Editors } from 'angular-slickgrid';
import { SaveBarDetailsModelDetailing } from 'src/app/Model/SaveBarDetailsModelDetailing';
...
import { Update_BPC_Structuremarking_Temp } from 'src/app/Model/Update_BPC_Structuremarking';
...
// import { GET_BPC_StructuremarkingList } from 'src/app/Model/GET_BPC_StructuremarkingList';
@Component({
  selector: 'app-bore-pile',
  templateUrl: './bore-pile.component.html',
  styleUrls: ['./bore-pile.component.css']
})


export class BorePileComponent {

  @ViewChild('myCanvas', { static: true })
  Canvas!: ElementRef<HTMLCanvasElement>;
  iseditable: boolean = false;
  BarSizeList: any[] = [];
  BarSizeList_backup: any[] = [];
  DetailingID: any;
  ...
  commonPopupModel!: BoredPilePoupModal;
  Insert_BPC_Structuremarking_Temp: Insert_BPC_Structuremarking_Temp = {
    intSEDetailingId: 0,
    vchStructureMarkingName: '',
    tntStructureRevNo: 0,
    intSAPMaterialCodeId: 0,
    intGroupMarkId: 0,
    tntGroupRevNo: 0,
    tntParamSetNumber: 0,
    intNoofWeldPoints: 0,
    intShenellTemplateId: 0,
    vchShenellAlternateTemplates: '',
    numPileDia: 0,
    numCageLength: 0,
    numCageDia: 0,
    vchFabricationType: '',
    ...
    vchElevationPattern: '',
    tntNumOfMainBar: 0,
    tntNumOfLinksatStart: 3,
    tntNumOfLinksatEnd: 2,
    tntAdditionalSpiral: 0,
    tntNumberOfStiffnerOrCentralizer: 0,
    tntNumOfSquareStiffner: 0,
    tntNumOfCircularRings: 0,
    intLapLength: 0,
    intEndLength: 0,
    intCoverToLink: 0,
    intAjustmentFactor: 0,
    bitCageNoteChangeIndicator: false,
    tntCageNoteId: 0,
    vchCageNote: '',
    MachineTypeId: 0,
    intMemberQty: 1,
    ...
    chrDrawingVersion: '',
    vchDrawingRemarks: '',
    vchTactonConfigurationState: '',
    vchHoleConfigurationXML: '',
    vchTactonXmlResult: '',
    bitAssembly: false,
    bitGenerate3DforPreview: false,
    nvchEdwgPath: '',
    chrGeneration3DStatus: '',
    vchOutputDrawingRef: '',
    bitCoat: false,
    vchMainBarDia: '',
    vchLinkDia: '',
    ...
    intCageDia: 0,
    intMachineTypeId: 0,
    vch3DImageRef: undefined,
    vchBPCPdfPath: undefined,
    numStructActualWeight: undefined,
    intCreatedUID: undefined,
    intUpdatedUID: undefined,
    sitProductTypeId: undefined,
    intSmallerMainBarLength: undefined,
    intBPCMaterialCodeID: undefined
  }
  ...
  gridOptions: {
    editable: boolean; enableAutoResize: boolean; enableCellNavigation: boolean; enableColumnReorder: boolean; enableColumnPicker: boolean; enableSorting: boolean; enableFiltering: boolean; createPreHeaderPanel: boolean; showPreHeaderPanel: boolean; preHeaderPanelHeight: number; explicitInitialization: boolean; enableRowSelection: boolean; enableAutoTooltip: boolean; enableAddRow: boolean; enableContextMenu: boolean; presets: {
  ...
  CouplerList: any[] = ["No Coupler", "E-Splice(S)", "N-Splice"];
  CouplerType: any = 'N-Splice';
  ...
  constructor(public httpClient: HttpClient,
    ...
    private BPCPatametersetService: BorePileService,
    ...
    this.gridOptions = {
      ...
      presets: {
        ...
        columns: [
          { columnId: 'id' },
          // { columnId: 'Cancelled' },
          { columnId: 'ElementMark' },
          { columnId: 'BarMark' },
          { columnId: 'BarType' },
          { columnId: 'BarSize' },
          { columnId: 'BarSTD' },
          //{ columnId: 'BarMemberQty' },
          { columnId: 'BarEachQty' },
          //{ columnId: 'BarTotalQty' },
          { columnId: 'BarShapeCode' },
          { columnId: 'A' },
          { columnId: 'B' },
          { columnId: 'C' },
          { columnId: 'D' },
    ...
    this.templateColumns = [
      ...
      //   minWidth: 0,
      //   width: 0,
      //   formatter: this.memberQtyFormatter,
      //   cssClass: 'right-align grid-text-size',
      // },
      {
        id: 'BarEachQty',
    ...
    // this.LoadGroupMarkIDPRC(this.MeshData.INTGROUPMARKID);
    // this.LoadSPAMaterialForPRC();
    // this.LoadProductCodeCoreCage();
    // this.SelectedCoreCageProductCode(this.MeshData.INTGROUPMARKID);
  ...
  async ngOnInit() {
    ...
    console.log("storedObjectData", this.storedObjectData);
    console.log("this.MeshData.INTSEDETAILINGID;", this.MeshData.INTSEDETAILINGID);
    ...
    this.tntParameterset = this.MeshData.TNTPARAMSETNUMBER
    ...
    this.setDefaultData();
    this.GetTableData();
    this.getShapeCodeList(this.CustomerCode, this.ProjectCode, 'N-Splice');
    this.BarSizeList = this.convertStrToObj(this.gDiaAll);
    //this.UpdateBarSize();
    debugger;
    this.getgriddata(this.MeshData.VCHGROUPMARKINGNAME, this.ProjectId, this.seDetailingID);
  ...
  ngOnDestroy() {
    debugger;
    localStorage.removeItem('MainBarList');
    localStorage.removeItem('additionalSpiralList');
    localStorage.removeItem('circularRingObj');
    localStorage.removeItem('ElevationList');
    localStorage.removeItem('StiffnerList');
    localStorage.removeItem('machineList');
  }
  getgriddata(GroupMarkingName: any, intProjectId: any, SELevelDetailsID: any) {
    this.BPCService
      .PopulateStructGrid(GroupMarkingName, intProjectId, SELevelDetailsID)
      .subscribe({
        next: (response) => {
          ...
          this.tabledata.forEach((element: any) => {
            ...
            if (element.vchTactonConfigurationState != "") {
              element.vchTactonConfigurationState = JSON.parse(element.vchTactonConfigurationState);
            } else {
              element.vchTactonConfigurationState = {}
            }
          })
        ...
        complete: () => {
          // JSON.parse(JSON.stringify(this.backup_update));
          let myString: string = "HelloWorld";
          let lastThreeChars: string = myString.substring(myString.length - 3);

          console.log(lastThreeChars);

          this.tabledata.forEach(element => {
            element.lastThreeChars = element.vchStructureMarkingName.substring(element.vchStructureMarkingName.length - 3)
            element.stringWithoutLastThreeChars = element.vchStructureMarkingName.slice(0, -3);
          });
          console.log("this.tabledata", this.tabledata);
          this.tabledata_back = JSON.parse(JSON.stringify(this.tabledata));
          // this.loading = false;
        },
      });
  }
  ...
  LoadParameterSetList_BPC(projectId: any) {
    ...
    this.BPCPatametersetService.GetParameterSetLis_BorePile(projectId).subscribe({
      next: (response) => {
      ...
      complete: () => {
        ...
        if (this.ParameterSetList.length) {
          this.ParameterValues = this.ParameterSetList.find((x: any) => x.tntParamSetNumber === this.tntParameterset);
          this.Insert_BPC_Structuremarking_Temp.intCoverToLink = this.ParameterValues.intcovertolink;
          this.Insert_BPC_Structuremarking_Temp.intAjustmentFactor = this.ParameterValues.intadjfactor;
          this.Insert_BPC_Structuremarking_Temp.intLapLength = this.ParameterValues.intlaplength;
          this.Insert_BPC_Structuremarking_Temp.intEndLength = this.ParameterValues.intendlength;

          this.Load_GetBorePilePopulateMethods("FABMETHODS", this.ParameterValues.sitProductTypel2Id, " A")
          // this.Load_GetBorePilePopulateMethods("ELEVATIONPATTERN",this.ParameterValues.sitProductTypel2Id," A")
          this.Load_GetBorePilePopulateMethods("MAINBARPATTERN", this.ParameterValues.sitProductTypel2Id, " A")
          this.Load_GetBorePilePopulateMethods("MACHINETYPE", this.ParameterValues.sitProductTypel2Id, " A")

          console.log("BPC parameter set ", this.ParameterValues);
        }
      },
    });
  ...
  Load_ProductType() {
    this.BPCPatametersetService.GetProductType_BorePile()
      .subscribe({
        next: (response) => {
          debugger;
  ...
  Load_GetBorePilePopulateMethods(strType: any, intProductL2Id: any, strMainBarCode: any) {
    this.BPCService.GetBorePilePopulateMethods(strType, intProductL2Id, strMainBarCode)
      .subscribe({
        next: (response) => {
          debugger;
          if (strType == "FABMETHODS") {
            this.FABMETHODS_list = response;
            console.log("FABMETHODS", this.FABMETHODS_list);
          }
          else if (strType == "ELEVATIONPATTERN") {
            debugger;
            this.ELEVATIONPATTERN_list = response;
            console.log("ELEVATIONPATTERN", this.ELEVATIONPATTERN_list);
          }
          else if (strType == "MAINBARPATTERN") {
            this.MAINBARPATTERN_list = response;
            console.log("MAINBARPATTERN ", this.MAINBARPATTERN_list);
          }
          else if (strType == "MACHINETYPE") {
            this.machineTypeList = response;
            console.log("MACHINETYPE ", this.machineTypeList);
          }
        ...
        complete: () => {
          if (strType == "FABMETHODS") {
            this.Insert_BPC_Structuremarking_Temp.vchFabricationType = this.FABMETHODS_list[0].vchDescription;
         
          }

          else if (strType == "MAINBARPATTERN") {
            this.Insert_BPC_Structuremarking_Temp.vchMainBarPattern = this.MAINBARPATTERN_list[0].vchMainBarPatternCode;
            this.changeMainBar(this.Insert_BPC_Structuremarking_Temp.vchMainBarPattern);
            console.log("MAINBARPATTERN ", this.MAINBARPATTERN_list);
          }
          else if (strType == "ELEVATIONPATTERN") {
            this.Insert_BPC_Structuremarking_Temp.vchElevationPattern = this.ELEVATIONPATTERN_list[0].vchElevationCode;
            console.log("ELEVATIONPATTERN", this.ELEVATIONPATTERN_list);
          }
          else if (strType == "MACHINETYPE") {
            if(intProductL2Id==32)            {
              if(intProductL2Id=32)
              this.Insert_BPC_Structuremarking_Temp.MachineTypeId = 5;

            }
            else{
              this.Insert_BPC_Structuremarking_Temp.MachineTypeId = 4;
  ...
  changeMainBar(strBartype: any) {
    this.ELEVATIONPATTERN_list = [];
    this.Insert_BPC_Structuremarking_Temp.vchElevationPattern = ""
    this.Load_GetBorePilePopulateMethods("ELEVATIONPATTERN", this.ParameterValues.sitProductTypel2Id, strBartype)
  }
  Load_PopulateCageNotes() {
    this.BPCService.PopulateCageNotes()
      .subscribe({
        ...
        complete: () => {
          this.Insert_BPC_Structuremarking_Temp.tntCageNoteId = this.CageNotes_list[0].tntNoteID;

          localStorage.setItem('cageNote', this.CageNotes_list[0].txtCageNote);
  ...
  fnVal(item: any): any {
    ...
    debugger
    // var MBLen = document.getElementById(mainBarLen).value;
    // var SPLen = document.getElementById(elevationLen).value;
    var MBLen = 0;
    var SPLen = 0;
    var LapLen = item.intLapLength
    var EndLen = item.intEndLength;
    var sumLength = Number(SPLen) + Number(LapLen) + Number(EndLen);


    if (item.vchStructureMarkingName == '') {
      alert('Please enter group mark')
      return false;
    }
    if (item.vchStructureMarkingName.length < 3) {
      alert('Please enter last 3 characters for structure marking name.')
      return false;
    }
    if (item.numPileDia == 0) {
      alert('Please enter Pile dia.')
      return false;
    }
    if (item.tntNumOfMainBar == 0) {
      alert('Please enter number of Main Bar.')
      return;
    }
    if (item.tntNumOfLinksatStart == 0) {
      alert('Please enter links at start.')
      return false;
    }
    if (item.tntNumOfLinksatEnd == 0) {
      alert('Please enter links at end.')
      return false;
    }
    if (item.tntAdditionalSpiral == null) {
      alert('Please enter additional spiral.')
      return false;
    }
    if (item.tntNumberOfStiffnerOrCentralizer == null) {
      alert('Please enter number of Stiffner rings.')
      return false;
    }
    if (item.tntNumOfSquareStiffner == null) {
      alert('Please enter number of Square stiffner.')
      return false;
    }
    if ((item.tntNumberOfStiffnerOrCentralizer != 0) && (item.tntNumOfSquareStiffner != 0)) {
      var numSqStf = item.tntNumOfSquareStiffner;
      var numReminder = (Number(numSqStf) % 4);
      if (numReminder != 0) {
        alert('Square stiffner value should be multiple of 4 !');
        return false;
      }
    }
    if (item.tntNumOfCircularRings == null) {
      alert('Please enter number of Circular Ring.')
      return false;
    }
    if (item.numCageLength == 0) {
      alert('Please enter Cage length.')
      return false;
    }
    if (item.intMemberQty == 0) {
      alert('Please enter Member quantity.')
      return false;
    }


    if (Number(item.numPileDia) <= Number(2 * (Number(item.intCoverToLink)))) {
      alert('Pile diameter can not be less than or equal to twice the Cover to link.');
      return false;
    }

    // if (mode != 'A') {

    //   if (document.getElementById(hdnPopUpMainBarDia).value == '0') {
    //     alert('Please select main bar dia by clicking the V link under main bar pattern.');
    //     return false;
    //   }
    //   if ((document.getElementById(hdnPopUpLinkDia).value == '0') || (document.getElementById(hdnPopUpLinkPitch).value == '0')) {
    ...
    // if (mode == 'A') {
    //   if (document.getElementById(hdnPopUpMainBarDia).value == '') {
    //     alert('Please select main bar dia by clicking the V link under main bar pattern.');
    //     return false;
    //   }

    //   if ((document.getElementById(hdnPopUpLinkDia).value == '') || (document.getElementById(hdnPopUpLinkPitch).value == '')) {
    //     alert('Please select link dia and pitch by clicking the V link under elevation pattern.');
    //     return false;
    //   }
    // }
    // if (sumLength != MBLen) {
    //   var check = confirm('Sum of spiral link length, lap length and end length is not equal to MainBar length. \n Do you want to continue ?');
    //   if (!check) {
    //     return false;
    //   }
    // }

    var strFabMethod = item.vchFabricationType;
    if (strFabMethod != 'Micro Pile') {
      if (Number(LapLen) < 700) {
        alert('Lap length cannot be less than 700 ');
        return false;
      }
      if (Number(EndLen) < 500) {
        alert('End length cannot be less than 500 ');
        return false;
      }
    }
  ...
  GenerateDrawingXML(
    ddlFabMethods: string,
    ddlMainBar: string,
    ddlElevationPattern: string,
    strStructureMarkingName: string,
    txtAddPileDia: string,
    txtAddLapLength: string,
    txtAddEndLength: string,
    txtCageLength: any,
    txtAddCover: string,
    txtAddNoLinksStart: string,
    txtAddNoLinksEnd: string,
    arg11: number,
    refMainBarList: any[],
    refSpiralLinkList: any[],
    dsBPCXML: any,
    intGrpId: number,
    refStiffnerList: any[],
    refCircularList: any[],
    refCentralizerList: any[]
  ) {
  ...
  CalculateNoOfWeldPoints(
    mainBarCount: any,
    linksStart: any,
    linksEnd: any,
    addSpiralCount: any,
    stiffRingCount: any,
    squareStiffCount: any,
    intSmallerMainBarLength: any,
    mainBarList: any,
    spiralLinkList: any,
    stiffenerList: any,
    circularRingList: any,
    centralizerList: any
  ) {
    let strSafeState: string
    let intRow: number;
    let blnfirst: boolean
    let intSpiralWeldCount: number = 0
    let intNoOfWeldPoints: number = 0
    // Dim BPCJsonObj As New List(Of BPCJSONParser)
    // ViewState("strSafeState") = Session("MainBarEditData")


    let addSpiralListJson = localStorage.getItem("additionalSpiralList");
    let mainBarListJson = localStorage.getItem("MainBarList");
    let spiralLinkListJson = localStorage.getItem("ElevationList");

    let addSpiralList = JSON.parse(addSpiralListJson!);
    mainBarList = JSON.parse(mainBarListJson!);
    spiralLinkList = JSON.parse(spiralLinkListJson!);


    if (spiralLinkList.length <= 0)
      blnfirst = true
    // else if (spiralLinkList.Item(1).ToString.Contains("-1000000"))
    // 	blnfirst = true
    let pitch = 0;
    let length = 0;
    spiralLinkList.forEach((_element: any) => {
      pitch = _element.spiral_link_part_pitch_field
      length = _element.spiral_link_part_length_field
      intSpiralWeldCount = intSpiralWeldCount + (mainBarCount * (length / pitch))
    });

    // let intMBLen = 0;
    // intSmallerMainBarLength = mainBarList[0].main_bar_part_length_field


    // mainBarList.forEach((_element: any) => {

    //   intMBLen = _element.main_bar_part_length_field;

    //   if (intMBLen <= intSmallerMainBarLength)
    //     intSmallerMainBarLength = intMBLen
    // });



    intNoOfWeldPoints = Number(round(intSpiralWeldCount + ((linksStart + linksEnd + addSpiralCount) * mainBarCount) + (stiffRingCount * mainBarCount) + (squareStiffCount * 2)))
    return intNoOfWeldPoints
  }


  async onsubmit(item: Insert_BPC_Structuremarking_Temp) {
    if (this.fnVal(item)) {
      debugger;
      let strPileDia: string = '';
      let strLinkPitch: string = '';
      let strCageLength: string = '';
      let strNoMainBar: string = '';
      let strCoverCode: string = '';
      let intPitchCount: number = 0;
      let txtCover = item.intCoverToLink.toString();
      let hdnPopUpLinkPitch = ''; //temp
      let txtAddPileDia = ''; //temp

      strCoverCode = await this.getCoverCodeForStrucMarking_Wrapper(Number(item.intCoverToLink));



      // Set pile diameter
      // let strPileDia: string = "";
      const pileDiaInput: number = this.Insert_BPC_Structuremarking_Temp.numPileDia;
      const pileDia: number = pileDiaInput / 10;

      if (pileDia.toString().includes('.')) {
        if (Math.round(pileDia).toString().length === 1) {
          strPileDia = '00' + Math.round(pileDia).toString();
        } else if (Math.round(pileDia).toString().length === 2) {
          strPileDia = '0' + Math.round(pileDia).toString();
        } else if (Math.round(pileDia).toString().length >= 3) {
          strPileDia = Math.round(pileDia).toString();
        }
      } else {
        if (pileDia.toString().length === 1) {
          strPileDia = '00' + pileDia.toString();
        } else if (pileDia.toString().length === 2) {
          strPileDia = '0' + pileDia.toString();
        } else if (pileDia.toString().length >= 3) {
          strPileDia = pileDia.toString();
        }
      }

      // Set link pitch
      // let strLinkPitch: string = "";
      const linkPitchInput: number = parseInt(this.Insert_BPC_Structuremarking_Temp.vchLinkPitch); //hdnPopUpLinkPitch.Value
      const linkPitch: number = linkPitchInput / 10;

      if (linkPitch.toString().includes('.')) {
        if (Math.round(linkPitch).toString().length === 1) {
          strLinkPitch = '0' + Math.round(linkPitch).toString();
        } else if (Math.round(linkPitch).toString().length >= 2) {
          strLinkPitch = Math.round(linkPitch).toString();
        }
      } else {
        if (linkPitch.toString().length === 1) {
          strLinkPitch = '0' + linkPitch.toString();
        } else if (linkPitch.toString().length >= 2) {
          strLinkPitch = linkPitch.toString();
        }
      }

      // Check and update strLinkPitch
      if (strLinkPitch !== '' && hdnPopUpLinkPitch.toString() !== '') {
        if (strLinkPitch.length > 2) {
          if (hdnPopUpLinkPitch.toString().trim().length >= 2) {
            strLinkPitch = hdnPopUpLinkPitch.toString();
          }
        }
      }
      ...
      if (item.numCageLength >= 1300 && item.numCageLength <= 9999) {
        strCageLength = item.numCageLength.toString().substring(0, 2);
      } else if (item.numCageLength >= 1000 && item.numCageLength <= 1299) {
        strCageLength = '0' + item.numCageLength.toString().substring(0, 1);
      } else {
        strCageLength = item.numCageLength.toString().substring(0, 2);
      }

      // let strNoMainBar: string = "";
      const numOfMainBar: number = item.tntNumOfMainBar;

      if (numOfMainBar.toString().length === 1) {
        strNoMainBar = '0' + numOfMainBar.toString();
      } else if (numOfMainBar.toString().length === 2) {
        strNoMainBar = numOfMainBar.toString();
      } else if (numOfMainBar.toString().length > 2) {
        if (numOfMainBar <= 109) {
          strNoMainBar = this.commonPopupModel.MainBars109 ?? '';
        } else if (numOfMainBar <= 119) {
          strNoMainBar = this.commonPopupModel.MainBars119 ?? '';
        } else if (numOfMainBar <= 129) {
          strNoMainBar = this.commonPopupModel.MainBars129 ?? '';
        } else if (numOfMainBar <= 139) {
          strNoMainBar = this.commonPopupModel.MainBars139 ?? '';
        } else if (numOfMainBar <= 149) {
          strNoMainBar = this.commonPopupModel.MainBars149 ?? '';
        } else if (numOfMainBar <= 159) {
          strNoMainBar = this.commonPopupModel.MainBars159 ?? '';
        } else if (numOfMainBar <= 169) {
          strNoMainBar = this.commonPopupModel.MainBars169 ?? '';
        } else if (numOfMainBar <= 179) {
          strNoMainBar = this.commonPopupModel.MainBars179 ?? '';
        } else if (numOfMainBar <= 189) {
          strNoMainBar = this.commonPopupModel.MainBars189 ?? '';
        } else if (numOfMainBar <= 199) {
          strNoMainBar = this.commonPopupModel.MainBars199 ?? '';
        } else if (numOfMainBar > 200) {
          strNoMainBar = this.commonPopupModel.MainBars200 ?? '';
        }
      }

      let ddlElevationPattern = this.Insert_BPC_Structuremarking_Temp.vchElevationPattern;
      let strElvnType: string = '';
      let strDoubleElevtn: string = this.commonPopupModel.DoubleLink ?? '';
      let strSingleElevtn: string = this.commonPopupModel.SingleLink ?? '';

      // strElvnType = ddlElevationPattern.SelectedItem.Text.trim();  //dropdown value
      strElvnType = ddlElevationPattern;

      // let intPitchCount: number;

      if (strDoubleElevtn.trim().includes(strElvnType.trim())) {
        intPitchCount = 2;
      } else {
        intPitchCount = 1;
      }

      let ddlMainBar = this.Insert_BPC_Structuremarking_Temp.vchMainBarPattern; //dropdown
      // for MainBar Type handling
      // let strMainType: string = ddlMainBar.SelectedItem.Text.trim();
      let strMainType: string = ddlMainBar;
      let blnDouble: boolean = false;
      let strDoubleLayer: string = this.commonPopupModel.DoubleLayerMB ?? '';
      let strSingleLayer: string = this.commonPopupModel.SingleLayerMB ?? '';
      let hdnPopUpMainBarDia = this.Insert_BPC_Structuremarking_Temp.vchMainBarDia; //temp
      let hdnPopUpLinkDia = this.Insert_BPC_Structuremarking_Temp.vchLinkDia; //temp
      let txtAddStructlastthreedigits = this.Insert_BPC_Structuremarking_Temp.vchStructureMarkingName; //temp
      let txtAddCover = this.ParameterValues.intcovertolink;

      if (strDoubleLayer !== '' && strSingleLayer !== '') {
        if (strSingleLayer.trim().includes(strMainType.trim())) {
          blnDouble = false;
        }
        if (strDoubleLayer.trim().includes(strMainType.trim())) {
          blnDouble = true;
        }
      }


      let strStructureMarkingName: string = '';

      if (intPitchCount >= 2) {
        if (blnDouble) {
          strStructureMarkingName =
            strPileDia +
            strCoverCode +
            strNoMainBar +
            strMainType +
        ...
        item.vchStructureMarkingName = strStructureMarkingName;
      } else {
        if (blnDouble) {
          strStructureMarkingName =
            strPileDia +
      ...
      let intCageDia: number = 0;
      intCageDia = parseInt(txtAddPileDia, 10) - 2 * parseInt(txtAddCover, 10);


      // Checking for config XML when user did not click the popup
      // if (sessionStorage.getItem('HoleConfigurationXML') === null) {
      //   try {
      //     // this.CreateConfigurationXML(); //conversion pending
      //   } catch (ex) {
      //     sessionStorage.removeItem('HoleConfigurationXML');
      //   }
      // }

      // for adding no of weld points
      let intNoOfWeldPoints: number = 0;
      let intSmallerMainBarLength: number = parseInt(this.Insert_BPC_Structuremarking_Temp.intSmallerMainBarLength);
      let refMainBarList: any[] = [];
      let refSpiralLinkList: any[] = [];
      let refStiffnerList: any[] = [];
      let refCircularList: any[] = [];
      let refCentralizerList: any[] = [];
      let txtAddNoMainBar: string = this.Insert_BPC_Structuremarking_Temp.tntNumOfMainBar.toString(); //temp
      let txtAddNoLinksStart = this.Insert_BPC_Structuremarking_Temp.tntNumOfLinksatStart.toString();
      let txtAddNoLinksEnd = this.Insert_BPC_Structuremarking_Temp.tntNumOfLinksatEnd.toString(); //temp
      let txtAddNoAdditionalSpiral = this.Insert_BPC_Structuremarking_Temp.tntAdditionalSpiral.toString(); //temp
      let txtAddNoStiffnerRing = this.Insert_BPC_Structuremarking_Temp.tntNumberOfStiffnerOrCentralizer.toString(); //temp
      let txtAddNoSquareStiffner = this.Insert_BPC_Structuremarking_Temp.tntNumOfSquareStiffner.toString(); //temp
      intNoOfWeldPoints = this.CalculateNoOfWeldPoints(
        parseInt(txtAddNoMainBar, 10),
        parseInt(txtAddNoLinksStart, 10),
        parseInt(txtAddNoLinksEnd, 10),
        parseInt(txtAddNoAdditionalSpiral, 10),
        parseInt(txtAddNoStiffnerRing, 10),
        parseInt(txtAddNoSquareStiffner, 10),
        intSmallerMainBarLength,
        refMainBarList,
        refSpiralLinkList,
        refStiffnerList,
        refCircularList,
      ...
      // for alternate schnell templates and schnell template id
      let strMachine: string = '';
      let ddlMachineType = ''; //temp
      let hdnAlternateSnlTemplates = ''; //temp
      let hdnSnlTempId = '';

      // strMachine = ddlMachineType;

      // if (strMachine !== "") {
      //     if (strMachine.toLowerCase().includes("schnell")) {
      //         if (hdnAlternateSnlTemplates === "") {
      //             item.vchShenellAlternateTemplates = "0";
      //         } else {
      //             item.vchShenellAlternateTemplates = hdnAlternateSnlTemplates;
      //         }

      //         if (hdnSnlTempId === "") {
      //             item.intShenellTemplateId = 0;
      //         } else {
      //             item.intShenellTemplateId = parseInt(hdnSnlTempId, 10);
      //         }
      //     } else {
      //         item.vchShenellAlternateTemplates = "0";
      //         item.intShenellTemplateId = 0;
      ...
      let hdnChangeIndicator = ''; //temp
      let ddlCageNotes = ''; // temp might be dropdown
      //Commented by Tanmay
      if (sessionStorage.getItem('cageNote') === null) {
        let dsCagenote: any = localStorage.getItem('cageNote');
        item.vchCageNote = dsCagenote.toString();
      } else {
        item.vchCageNote = localStorage.getItem('cageNote')!.toString();
      }
      ...
      if (hdnChangeIndicator === '') {
        // item.bitCageNoteChangeIndicator = 0; //commented by me
        item.bitCageNoteChangeIndicator = false;
      } else {
        // item.bitCageNoteChangeIndicator = hdnChangeIndicator; /commented by me
      }

      let txtStructRevNo = '';
      let lblGrpVer = ''; //temp
      let ddlParameter = ''; // temp dropdown
      let ddlFabMethods = ''; //temp
      let hdnMPattern = ''; // temp
      let hdnEPattern = ''; //temp
      let txtNosCircularRing = ''; //temp
      let txtAddLapLength = ''; // temp
      let txtAddEndLength = ''; //temp
      let txtAddAdjustFactor = ''; //temp
      let txtAddMemQty = '';


      // objDetailInfo.intSELevelDetailsID = ViewState["SharedSEDetailingRefID"]; //commented by vw

      if (txtStructRevNo !== '') {

      } else {
        item.tntStructureRevNo = 0;
      }

      // let intGroupMarkId: number = GroupMarkIndex();
      item.intGroupMarkId = Number(this.IntGroupMarkId);;
      item.tntGroupRevNo = 0;
      item.tntParamSetNumber = this.tntParameterset
      item.intNoofWeldPoints = intNoOfWeldPoints;

      item.numCageDia = intCageDia;

      item.intMachineTypeId = parseInt(ddlMachineType, 10);

      item.numActualWeight = 0;
      item.vchDrawingReference = '';
      item.chrDrawingVersion = '';
      item.vchDrawingRemarks = '';

      let obj = {
        mainBarList: JSON.parse(localStorage.getItem('MainBarList')!),
        additionalSpiralList: JSON.parse(localStorage.getItem('additionalSpiralList')!),
        circularRingObj: JSON.parse(localStorage.getItem('circularRingObj')!),
        elevationList: JSON.parse(localStorage.getItem('ElevationList')!),
        stiffnerList: JSON.parse(localStorage.getItem('StiffnerList')!),
        machineList: JSON.parse(localStorage.getItem('machineList')!)
      }

      item.vchTactonConfigurationState = JSON.stringify(obj);
      item.vchHoleConfigurationXML = '';
      item.vchTactonXmlResult = '';
      // item.bitGenerate3DforPreview = 0; commented by me
      item.bitGenerate3DforPreview = false;
      item.vch3DImageRef = 0;
      item.chrGeneration3DStatus = 'P';
      item.vchOutputDrawingRef = '';
      item.vchBPCPdfPath = '';

      let chkCage: any; //temp
      let chkAssembly: any; //temp

      // if (chkCage.checked) {
      //   objDetailInfo.bitCoat = 1;
      // } else {
      //   objDetailInfo.bitCoat = 0;
      // }

      // if (chkAssembly.checked) {
      //   objDetailInfo.bitAssemblyIndicator = 1;
      // } else {
      //   objDetailInfo.bitAssemblyIndicator = 0;
      // }

      item.tntStatusId = 1;
      // item.intCreatedUID = parseInt(ViewState['UserId'], 10);
      // item.intUpdatedUID = parseInt(ViewState['UserId'], 10);
      item.intCreatedUID = 0;
      item.intUpdatedUID = 0;

      // added by debarati CHG0032240
      item.sitProductTypeId = parseInt(this.ParameterValues.sitProductTypel2Id);
      let mainBarList = JSON.parse(localStorage.getItem('MainBarList')!)
      if(mainBarList.length > 1){
        item.intSmallerMainBarLength = mainBarList.reduce((prev:any, current:any) => {
          return (parseInt(prev.main_bar_part_length_field) < parseInt(current.main_bar_part_length_field)) ? prev.main_bar_part_length_field : current.main_bar_part_length_field;
        });
      }else{
        item.intSmallerMainBarLength = mainBarList[0].main_bar_part_length_field;
      }
      intSmallerMainBarLength = parseInt(item.intSmallerMainBarLength);
      if (intSmallerMainBarLength !== 0) {
        item.intSmallerMainBarLength = intSmallerMainBarLength;
      } else {
        item.intSmallerMainBarLength = 0;
      }

      // strSAPCodeID = objDetailDal.getBPCMaterialCodeIDForStrucMarking(item); //commented by vw



      item.numStructActualWeight = 0
      item.intBPCMaterialCodeID = 12

      item.intLapLength = Number(item.intLapLength)
      item.intEndLength = Number(item.intEndLength)
      item.numPileDia = Number(item.numPileDia)


      item.vch3DImageRef = 0;
      item.vchBPCPdfPath = 0;
      item.intSEDetailingId = this.MeshData.INTSEDETAILINGID;
      item.intMachineTypeId = item.MachineTypeId
      let INVWEIGHT: number = 0;
      let PRODWEIGHT: number = 0;

      let strSAPCodeID
      strSAPCodeID = await this.getBPCMaterialCodeIDForStrucMarking_Wrapper(item.vchMainBarPattern, item.vchElevationPattern, item.intSmallerMainBarLength, item.numPileDia, item.bitCoat, item.sitProductTypeId)

      if (!isNaN(parseFloat(strSAPCodeID))) {
        item.intSAPMaterialCodeId = strSAPCodeID;

        let ds: any;
        ds = await this.GetCABDetails(item.intSEDetailingId, 0);
        if (ds.length) {
          ds.forEach((element: any) => {
            INVWEIGHT += element.INVWEIGHT
            PRODWEIGHT += element.PRODWEIGHT
          });
          item.numActualWeight = PRODWEIGHT;
          item.numNetWeight = INVWEIGHT;
        } else {
          item.numStructActualWeight = 0;
          item.numNetWeight = 0;
        }
      ...
      let strStructureElementType: string = 'Pile';
      let strConfigState: string | null = null;

      // letintCageDia = CInt(txtEditPileDia.Text) - 2 * CInt(txtEditCover.Text)
      item.numCageDia = item.numPileDia - 2 * item.intCoverToLink
      item.intCageDia = 0;

      item.vchLinkPitch = item.vchLinkPitch.toString();



      intOutput = this.InsertBPCStructureMarkingDetails(item);
      // intOutput = objDetailDal.InsertBPCStructureMarkingDetails(objDetailInfo); //commented by vw

      // try {
      //     using (let objTS: System.Transactions.TransactionScope = new System.Transactions.TransactionScope()) {
      //         intOutput = objDetailDal.InsertBPCStructureMarkingDetails(objDetailInfo);
      //         ViewState["StructureMarkingRefID"] = intOutput;

      //         if (ViewState["StructureMarkingRefID"] > 0) {
      //             Session["bpcstructid"] = parseInt(ViewState["StructureMarkingRefID"], 10);
      //             Session["grpid"] = intGrpId;
      //             Session["selevelid"] = parseInt(ViewState["SharedSEDetailingRefID"], 10);

      //             if (ds !== null && Session["ArmaId"] !== 0) {
      //                 // Add your logic here
      //             }

      //             ViewState["adn"] = null;
      //             let strDrawingXML: string = "";

      //             let objDrawing: BPCDrawing = new BPCDrawing();
      //             let dsBPCXML: DataSet = new DataSet();
      //             strConfigState = Session["HoleConfigurationXML"] !== null ? Session["HoleConfigurationXML"].toString() : null;
      //             let intSchnellId: number = hdnSnlTempId !== "" ? parseInt(hdnSnlTempId.trim(), 10) : 0;
      //             let intRevNumber: number = parseInt(txtStructRevNo, 10);
      //             let txtCageLength: any; //temp
      //             if (strConfigState !== null && strConfigState !== "") {
      //                 objDrawing.GenerateBPCDrawingPDF(ddlElevationPattern, strStructureMarkingName, txtAddPileDia, txtAddLapLength, txtAddEndLength, txtCageLength, txtAddCover, txtAddNoLinksStart, txtAddNoLinksEnd, intOutput, parseInt(ViewState["WBSElementId"], 10), intGroupMarkId, parseInt(ViewState["SharedSEDetailingRefID"], 10), refMainBarList, refSpiralLinkList, strMachine, dsBPCXML, strConfigState, intSchnellId, intRevNumber);
      //             }

      //             this.GenerateDrawingXML(ddlFabMethods, ddlMainBar, ddlElevationPattern, strStructureMarkingName, txtAddPileDia, txtAddLapLength, txtAddEndLength, txtCageLength, txtAddCover, txtAddNoLinksStart, txtAddNoLinksEnd, parseInt(intOutput.toString(), 10), refMainBarList, refSpiralLinkList, dsBPCXML, intGrpId, refStiffnerList, refCircularList, refCentralizerList);
      ...
      //     } else {
      //         if (intOutput === -1) {
      //             // Structure marking already exists message
      //         } else {
    ...
    // PopulateStructGrid();
    // else {
    //     ScriptManager.RegisterStartupScript(this, this.GetType(), "Msg", "<script>alert('The SAPMaterialCodeID '" + strSAPCodeID + "' does not exist !');</script>", false);
    // }
    // Session["AData"] = null;

    // Session["mainbarlist"] = null;
    // Session["spiralLinkList"] = null;
    // Session["stiffnerList"] = null;
    // Session["addSpiralList"] = null;
    // Session["centralizerList"] = null;
    // Session["circularRingList"] = null;

    // Additional code goes here

    // else {
    //     window.location.href = "~/HomeDetailer.aspx?Q=TO";
    // }
    else {
      this.tosterService.warning('PLease enter correct values ');
    }
  }

  InsertBPCStructureMarkingDetails(item: Insert_BPC_Structuremarking_Temp) {
    this.BPCService.InsertBPCStructureMarkingDetails(item).subscribe({
      next: (response) => {
        // console.log(response);
        // debugger;
        // this.ParameterSetList = response;
        ...
        if(response==-1)
        {
          this.tosterService.warning(`The setcode ${item.vchStructureMarkingName} is already present in the project ,please check `)
        }
        else{
      ...
      complete: () => {
        ...
        // localStorage.removeItem('StiffnerList');
        // localStorage.removeItem('machineList');
        this.Insert_BPC_Structuremarking_Temp.vchStructureMarkingName = ""
  ...
  async getBPCMaterialCodeIDForStrucMarking_Wrapper(vchMainBarPattern: any, vchElevationPattern: any, numSmallerMainBarLength: any, numPileDia: any, bitCoat: any, sitProductTypeL2Id: any): Promise<any> {
    try {
      var a = await this.BPCService.getBPCMaterialCodeIDForStrucMarking(vchMainBarPattern, vchElevationPattern, numSmallerMainBarLength, numPileDia, bitCoat, sitProductTypeL2Id).toPromise();
      return a;
    } catch (error) {
      return error;
    }
  }
  // getBPCMaterialCodeIDForStrucMarking(vchMainBarPattern:any,vchElevationPattern:any,numSmallerMainBarLength:any,numPileDia:any,bitCoat:any,sitProductTypeL2Id:any)

  // {
  //   this.BPCPatametersetService.getBPCMaterialCodeIDForStrucMarking(vchMainBarPattern,vchElevationPattern,numSmallerMainBarLength,numPileDia,bitCoat,sitProductTypeL2Id).subscribe({
  //     next: (response) => {
  //       // console.log(response);
  //       // debugger;
  //       // this.ParameterSetList = response;
  ...
  async getCoverCodeForStrucMarking_Wrapper(intCoverLink: any): Promise<any> {
    try {
      var a = await this.BPCService.getCoverCodeForStrucMarking(intCoverLink).toPromise();
      return a;
    } catch (error) {
      return error;
    }
  ...
  // getCoverCodeForStrucMarking_Wrapper(intCoverLink:any)
  // {
  //   this.BPCService.getCoverCodeForStrucMarking(intCoverLink).subscribe((response:any)=>{
  //     this.strCode = response[0];
  //     console.log("this.strCode",this.strCode)
  //   });

  // }
  viewModal_MainBar(insertItem: any) {

    const modalRef = this.modalService.open(MainBarPatternComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_ElevationPattern(insertItem: any) {

    const modalRef = this.modalService.open(ElevationPopupComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_additionalSpiral(insertItem: any) {

    const modalRef = this.modalService.open(AdditionalSpiralPopupComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_cageNotes(insertItem: any) {

    const modalRef = this.modalService.open(CageNotesPopupComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_circularRing(insertItem: any) {

    const modalRef = this.modalService.open(CircularRingPopupComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_stiffnerRing(insertItem: any) {

    const modalRef = this.modalService.open(StiffnerRingPopupComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  viewModal_MachineType(insertItem: any) {
    const modalRef = this.modalService.open(MachineTypeComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = insertItem;
  ...
  setDefaultData() {
    this.commonPopupModel = {
      AddSpiral: 0,
      CirculaRing: 0,
      DoubleLayerMB: 'M02,M03,M04,M05,M06,M07,M08,M09',
      DoubleLink:
        'SLS02,SLC04,SLC05,SLC06,SLS07,SLC08,SLC09,SLC12,SLC13,SLM11,SLM10,SLS20,SLS23,SLC21,SLX22,SLC24,SLX25,SLS26,SLC27,SLX28',
      LinksEnd: 2,
      LinksStart: 3,
      MainBars109: 'H0',
      MainBars119: 'H1',
      ...
      MainBars199: 'H9',
      MainBars200: 'HX',
      SingleLayerMB: 'M01,M02,M09',
      SingleLink: 'SLS01,SLC03',
      SquareStiffner: 0,
      Stiffner: 3,
    };
  ...
  async InsertBarMark(
    ...
    try {
      const data = await this.detailingService
        .Insert_BarMark(item, SEDetailingID, intGroupMarkID)
        .toPromise();
      return data;
    } catch (error) {
      console.error(error);
  ...
  async Insert_BarMark(pGridID: any, pRowID: any): Promise<any> {
    ...
    if (pGridID != null && pRowID != null) {
      ...
      if (lItem.BBSID > 0 && lItem.BarID > 0) {
        ...
        let obj: SaveBarDetailsModelDetailing = {
          ...
          BarSize: lItem.BarSize ? Number(lItem.BarSize) : 0,
          CabProductMarkID: lItem.CabProductMarkID ? lItem.CabProductMarkID : 0,
          BarMemberQty: lItem.BarMemberQty ? Number(lItem.BarMemberQty) : 0,
          BarEachQty: lItem.BarEachQty ? Number(lItem.BarEachQty) : 0,
          BarTotalQty: lItem.BarTotalQty ? Number(lItem.BarTotalQty) : 0,
          ...
          BarWeight: lItem.BarWeight ? Number(lItem.BarWeight) : 0,
          ...
          shapeAutoCalcFormula3: lItem.shapeAutoCalcFormula3
            ...
            : '',
          shapeTransport: lItem.shapeTransport ? Number(lItem.shapeTransport) : 0,
        ...
        let groupMarkId = Number(this.IntGroupMarkId); //Commented by ajit

        let response: any = await this.InsertBarMark(
          obj,
          this.seDetailingID,
          groupMarkId
        );
        ...
        if (response == false) {
          ...
          if (response.Issuccess == true) {
            // alert('save Bar Success');
            console.log("SAVE->", response);

            let lItem = gridArray.getDataItem(pRowID);
            lItem.CabProductMarkID = response.ProductmarkId;
  ...
  async InsertItem() {
    ...
    if (this.templateGrid.slickGrid.getActiveCell() != null) {
      ...
      if (lRowIndex < this.dataViewCAB.getLength()) {
        ...
        let result = await this.Insert_BarMark(pGridID, lRowIndex);
        if (result == true) {
          //Tab#, Row#
  ...
  async DeleteCabProductmarking(CABProductMarkName: any, CABProductMarkID: any): Promise<any> {
    try {
      let SEDetailingID = this.seDetailingID
      const data = await this.detailingService.DeleteCabProductmarking(SEDetailingID, CABProductMarkName, CABProductMarkID).toPromise();
      return data;
    } catch (error) {
      console.error(error);
      return false;
  ...
  async DeleteBarDetails() {
    ...
    if (lRows.length > 0) {
      lRows.sort(function (a: any, b: any) { return a - b });
      var lRowsNo = ""
      ...
      for (let i = lRows.length - 1; i >= 0; i--) {
        ...
        if (lItem != null) {
          ...
          // let response: any = await this.DeleteBarDetails_CAB(lCustomerCode, lProjectCode, lJobID, lbbsID, lBarID);

          // CAAL DELETE HERE
          let cabProductmarkId = lItem.CabProductMarkID;
          let cabProductmarkName = lItem.BarMark;
          let response: any = await this.DeleteCabProductmarking(cabProductmarkId, cabProductmarkName);
          ...
          if (response == false) {
            ...
            //   lSuccess = 0;
            //   if (response.ErrorMessage != null) {
            //     alert("Failed to delete the BBS details data. Error Message: " + response.ErrorMessage);
            //   } else {
            //     alert("Failed to delete the BBS details data.");
            //   }
            // }
  ...
  Delete() {
    var lRows = this.templateGrid.slickGrid.getSelectedRows();
    if (lRows.length > 0) {
      if (confirm("It going to delete the selected items. Are you sure?\n\n将要删除您所选择的行. 请确认?")) {
        // startLoading();
  ...
  async ClearRows() {
    ...
    if (
      ...
      for (let i = 0; i < lRows.length; i++) {
        let lItem: SaveBarDetailsModelDetailing = this.templateGrid.slickGrid.getDataItem(lRows[i]);
        ...
        lItem.BarSort = lItem.BarSort ? lItem.BarSort : 0;

        let id = this.templateGrid.slickGrid.getDataItem(lRows[i]).id;
        let groupMarkId = Number(this.IntGroupMarkId);

        let response: any = await this.InsertBarMark(lItem, this.seDetailingID, groupMarkId);
  ...
  CopyRows() {
    ...
    let copyItem = [];
    for (let i = 0; i < lRows.length; i++) {
      let item = this.templateGrid.slickGrid.getDataItem(lRows[i]);
      copyItem.push(item);
    }

    this.CopiedBBSRows = copyItem;
    console.log('item to be Copied ->', copyItem);
  }
  ...
  PasteRows() {
    ...
    for (var i = lData.length - 1; i >= 0; i--) {
      ...
      this.Insert_BarMark(0, lRowIndex); //Tab#, Row#
      // CALL SAVE FUNCTION
  ...
  async grid_onActiveCellChanged(e: any, args: any) {
    ...
    if (lItem.BarMemberQty != 1 || lItem.BarMemberQty != undefined) {
      let rowNum = this.templateGrid.slickGrid.getActiveCell().row;
      let lItem = this.templateGrid.slickGrid.getDataItem(rowNum);
      lItem.BarMemberQty = 1;
      this.dataViewCAB.updateItem(lItem.id, lItem);
    }
  ...
  grid_onSelectedRowsChanged(e: any, args: any) {
    ...
    if (args.rows.length > 0) {
      if (args.rows[0] != this.barRowIndex[this.gridIndex]) {
        this.Insert_BarMark(this.gridIndex, this.barRowIndex[this.gridIndex]); //Tab#, Row#
        this.displayShapeImage(this.gridIndex, args.rows[0]); // Commented by ajit
  ...
  async getShapeInfo(
    ...
    try {
      CustomerCode = this.CustomerCode; //'0001101170';
      ProjectCode = this.ProjectCode; //'0000113012';
      JobId = 986;
      const data = await this.orderService
  ...
  async getShapeImagesByCat_CAB(
    ...
    try {
      CustomerCode = this.CustomerCode; //'0001101170'; // added by ajit
      ProjectCode = this.ProjectCode; //'0000113012'; // added by ajit
      CouplerType = this.CouplerType; //  'N-Splice'; // added by ajit
      const data = await this.orderService
  ...
  async loadShapeInfo(pShapeCode: any, pGridID: any, pRowNo: any) {
    ...
    if (pShapeCode != null && pShapeCode.trim() != '') {
      ...
      if (this.bShapeCode.length > 0) {
        for (var i = 0; i < this.bShapeCode.length; i++) {
          if (pShapeCode == this.bShapeCode[i]) {
            ...
            this.displayShapeImage(pGridID, pRowNo); //commented by ajit
      ...
      if (lFoundShape == 0) {
        ...
        // SUCCESS
        if (response == false) {
          //alert('Get Shape information: Retrieve Database Error.'); commented by ajit
        } else {
          ...
          this.displayShapeImage(pGridID, pRowNo); //commmented by ajit
          //document.getElementById("rightShapeImage-" + pGridID).src = gShapeImage;
          //document.getElementById("btmShapeImage-" + pGridID).src = gShapeImage;
  ...
  async grid_onCellChange(e: any, args: any) {
    ...
    if (lColumnName == 'BarMemberQty' || lColumnName == 'BarEachQty') {
      var lMQty = 1;// args.item['BarMemberQty'];
    ...
    if (lColumnName.length == 1) {
      ...
      this.displayShapeImage(this.gridIndex, grid.getActiveCell().row); //commneted by ajit
    }
  ...
  async GetCABProductMarkingDetails(seDetailingID: number): Promise<any> {
    try {
      const data = await this.detailingService
        .GetCABProductMarkingDetailsByID(seDetailingID)
        .toPromise();
      return data;
    } catch (error) {
      console.error(error);
  ...
  async GetTableData() {
    let seDetailingID = this.seDetailingID; //2396114
    //let response = await this.GetBarDetails(CustomerCode, ProjectCode, JobID, BBSID);
    let response = await this.GetCABProductMarkingDetails(seDetailingID);
      ...
      if (response.length > 0) {
        for (let i = 0; i < response.length; i++) {
          ...
          data[i] = {
            ...
            CabProductMarkID: response[i].CabProductMarkID,
  ...
  CheckBBSDetails() {
    ...
    for (let j = 0; j < this.dataViewCAB.getLength(); j++) {
      ...
      if (this.dataViewCAB.getItem(j).Cancelled != true) {
        ...
        if (lMaxLength > lLenLimit) {
          let lTotalDed: any = this.getCreepDedution(
            lMaxLength,
  ...
  getShapeCodeList(
    ...
    console.log('CouplerType', CouplerType);
    this.orderService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.ShapeCatList = [...new Set(this.ShapeCatList)];
          this.ShapeCatList.push('All Shapes');
          if (this.CouplerType != 'N-Splice') {
  ...
  UpdateBPCStructureMarkingDetails(item: Update_BPC_Structuremarking_Temp) {
    this.BPCService.UpdateBPCStructureMarkingDetails(item).subscribe({
      next: (response) => {
          if(response==-1)
          {
            this.tosterService.warning(`The setcode ${item.vchStructureMarkingName} is already present in the project ,please check `)
            ...
            this.tosterService.success("Update successfully!");
          }
      },
      error: (e) => {
      },
  ...
  updateBpcExtraLayer(item: any) {
    let UpdateNew: Update_BPC_Structuremarking_Temp = {
      intStructureMarkId: item.intStructureMarkId,
      vchStructureMarkingName: item.lastThreeChars,
      tntStructureRevNo: item.tntStructureRevNo,
      intBPCMaterialCodeID: 0,
      intGroupMarkId: item.intGroupMarkId,
      tntGroupRevNo: item.tntGroupRevNo,
      tntParamSetNumber: item.tntParamSetNumber,
      intCageSeqNo: item.tntCageSeqNo,
      intNoofWeldPoints: item.intNoofWeldPoints,
      intShenellTemplateId: item.intShenellTemplateId,
      vchShenellAlternateTemplates: item.vchShenellAlternateTemplates,
      numPileDia: item.numPileDia,
      numCageLength: item.numCageLength,
      intCageDia: item.numCageDia,
      vchFabricationType: item.vchFabricationType,
      vchMainBarPattern: item.vchMainBarPattern,
      vchElevationPattern: item.vchElevationPattern,
      tntNumOfMainBar: item.tntNumOfMainBar,
      tntNumOfLinksatStart: item.tntNumOfLinksatStart,
      tntNumOfLinksatEnd: item.tntNumOfLinksatEnd,
      tntAdditionalSpiral: item.tntAdditionalSpiral,
      tntNumberOfStiffnerOrCentralizer: item.tntNumberOfStiffnerOrCentralizer,
      tntNumOfSquareStiffner: item.tntNumOfSquareStiffner,
      tntNumOfCircularRings: item.tntNumOfCircularRings,
      intLapLength: item.intLapLength,
      intEndLength: item.intEndLength,
      intCoverToLink: item.intCoverToLink,
      intAjustmentFactor: item.intAjustmentFactor,
      bitCageNoteChangeIndicator: item.bitCageNoteChangeIndicator,
      tntCageNoteId: item.tntCageNoteId,
      vchCageNote: item.vchCageNote,
      intMachineTypeId: item.MachineTypeId,
      intMemberQty: item.intMemberQty,
      numNetWeight: item.numNetWeight,
      numStructActualWeight: item.numActualWeight,
      ...
      vchTactonConfigurationState: item.vchTactonConfigurationState,
      vchHoleConfigurationXML: item.vchHoleConfigurationXML,
      vchTactonXmlResult: item.vchTactonXmlResult,
      bitAssemblyIndicator: item.bitAssemblyIndicator,
      bitGenerate3DforPreview: item.bitGenerate3DforPreview,
      vch3DImageRef: item.chrGeneration3DStatus,
      chrGeneration3DStatus: item.chrGeneration3DStatus,
      vchOutputDrawingRef: item.vchOutputDrawingRef,
      bitCoat: false,
      vchMainBarDia: item.vchMainBarDia,
      vchLinkDia: item.vchLinkDia,
      vchLinkPitch: item.vchLinkPitch,
      tntStatusId: item.tntStatusId,
      intCreatedUID: item.intCreatedUId,
      intUpdatedUID: item.intCreatedUId,
      vchBPCPdfPath: "",
      intSmallerMainBarLength:item.intSmallerMainBarLength
    }
    this.updateBpc(UpdateNew);
  }



  async GetCABDetails(SELevelDetailsID: any, intStructureMarkId: any): Promise<any> {
    try {
      var a = await this.BPCService.GetCABDetails(SELevelDetailsID, intStructureMarkId).toPromise();
      return a;
    } catch (error) {
      return error;
    }
  ...
  async updateBpc(item: Update_BPC_Structuremarking_Temp) {
    let intOutput: any;
    let strSAPCodeID: any;
    let txtEditSeqNo: any;
    let txtEditPileDia: any;
    let txtCageLength: any;
    let txtEditNoMainBar: any;
    let txtEditNoStiffnerRing: any;
    let txtNosCircularRing: any;
    let ddlFabMethods: any;
    let ddlMainBar: any;
    let ddlElevationPattern: any;
    let txtEditNoSquareStiffner: any;
    let txtEditLapLength: any;
    let txtEditEndLength: any;
    let txtEditAdjustFactor: any;
    let ddlMachineType: any;
    let chkCage: any;
    let chkAssembly: any;
    let txtEditCover: any;
    let lblStructId: any;
    let txtEditNoLinksStart: any;
    let txtEditNoLinksEnd: any;
    let txtEditNoAdditionalSpiral: any;
    let txtEditStructlastthreedigits: any;
    let txtEditMemQty: any;
    let lblSnlTemplateId: any;
    let lblMainBarDia: any;
    let lblLinkDia: any;
    let lblLinkPitch: any;
    let lblCageNote: any;
    let lblStar: any;
    let lblChangeIndicator: any;
    let ddlCageNotes: any;
    let lblAlternateSnlTemplates
    // StructRevNo is not defined in the provided code snippet

    // Assuming 'item' is an object with properties corresponding to the variables being assigned

    let StructRevNo = item.tntStructureRevNo;

    lblMainBarDia = item.vchMainBarDia;
    lblLinkDia = item.vchLinkDia;
    lblLinkPitch = item.vchLinkPitch;
    lblSnlTemplateId = item.intShenellTemplateId;
    lblAlternateSnlTemplates = item.vchShenellAlternateTemplates;
    lblCageNote = item.vchCageNote;

    // Assigning values from the popup window
    // Please ensure these elements are correctly referenced in your HTML

    // hdnPopUpMainBarDia = document.getElementById("hdnPopUpMainBarDia");
    // hdnPopUpLinkDia = document.getElementById("hdnPopUpLinkDia");
    // hdnPopUpLinkPitch = document.getElementById("hdnPopUpLinkPitch");
    // hdnChangeIndicator = document.getElementById("hdnChangeIndicator");
    // hdnAlternateSnlTemplates = document.getElementById("hdnAlternateSnlTemplates");
    // hdnGenerateImage = document.getElementById("hdnGenerateImage");

    // Find grid controls in footer
    txtEditSeqNo = item.intCageSeqNo;
    txtEditStructlastthreedigits = item.vchStructureMarkingName;
    txtEditPileDia = item.numPileDia;
    txtCageLength = item.numCageLength;
    txtEditNoMainBar = item.tntNumOfMainBar;
    txtEditNoStiffnerRing = item.tntNumberOfStiffnerOrCentralizer;
    txtNosCircularRing = item.tntNumOfCircularRings;
    ddlFabMethods = item.vchFabricationType;
    ddlMainBar = item.vchMainBarDia;
    ddlElevationPattern = item.vchElevationPattern;
    txtEditNoLinksStart = item.tntNumOfLinksatStart;
    txtEditNoLinksEnd = item.tntNumOfLinksatEnd;
    txtEditNoAdditionalSpiral = item.tntAdditionalSpiral;
    txtEditNoSquareStiffner = item.tntNumOfSquareStiffner;
    txtEditLapLength = item.intLapLength;
    txtEditEndLength = item.intEndLength;
    txtEditAdjustFactor = item.intAjustmentFactor;
    ddlMachineType = item.intMachineTypeId;
    chkCage = item.bitCageNoteChangeIndicator;
    chkAssembly = item.bitAssemblyIndicator;
    txtEditCover = item.intCoverToLink;
    txtEditMemQty = item.intMemberQty;
    lblStructId = item.intStructureMarkId;
    lblStar = ""; // Remaining
    lblChangeIndicator = item.bitCageNoteChangeIndicator;
    ddlCageNotes = item.vchCageNote;

    // Assign other variables
    // let strPileDia: string = "";
    let strLinkPitch: string = (Math.floor((Number(lblLinkPitch) / 10))).toString();
    let strCageLength: string = "";
    // let strNoMainBar: string = "";
    let strCoverCode: string = "";
    let intPitchCount: number = 0;
    ...
    // Set pile dia
    let strPileDia: string = (parseInt(txtEditPileDia) / 10).toString().includes(".") ?
      ("00" + Math.round(parseInt(txtEditPileDia) / 10).toString()) :
      (parseInt(txtEditPileDia) / 10).toString().padStart(3, '0');

    // Set cage length
    // let strCageLength: string;
    const cageLengthValue: number = parseInt(txtCageLength);
    if (cageLengthValue >= 1300 && cageLengthValue <= 9999) {
      strCageLength = txtCageLength.toString().substring(0, 2);
    } else if (cageLengthValue >= 1000 && cageLengthValue <= 1299) {
      strCageLength = "0" + txtCageLength.toString().substring(0, 1);
    } else {
      strCageLength = txtCageLength.toString().substring(0, 2);
    }

    // Set no main bar
    let strNoMainBar: any;
    const noMainBarValue: number = parseInt(txtEditNoMainBar);
    if (noMainBarValue.toString().length === 1) {
      strNoMainBar = "0" + noMainBarValue.toString();
    } else if (noMainBarValue.toString().length === 2) {
      strNoMainBar = noMainBarValue.toString();
    } else {
      // Handle different cases as in VB.NET code
    }

    let strMainType: string = item.vchMainBarPattern;
    let blnDouble: boolean = false;
    let strDoubleLayer: string = this.commonPopupModel.DoubleLayerMB ?? '';;
    let strSingleLayer: string = this.commonPopupModel.SingleLayerMB ?? '';;
    ...
    // Continue converting the rest of the VB.NET code to TypeScript
    // TypeScript equivalent code
    let strStructureMarkingName: string = '';
    let hdnPopUpLinkDia = item.vchLinkDia; //temp

    if (intPitchCount >= 2) {
      if (blnDouble) {
        strStructureMarkingName = strPileDia + strCoverCode + strNoMainBar + strMainType + strElvnType + "-" + strCageLength + txtEditStructlastthreedigits;
      } else {
        strStructureMarkingName = strPileDia + strCoverCode + strNoMainBar + lblMainBarDia + strElvnType + "-" + strCageLength + txtEditStructlastthreedigits;
      }
      item.vchStructureMarkingName = strStructureMarkingName;
    } else {
      if (blnDouble) {
        strStructureMarkingName = strPileDia + strCoverCode + strNoMainBar + strMainType + hdnPopUpLinkDia + strLinkPitch + "-" + strCageLength + txtEditStructlastthreedigits;
      } else {
        strStructureMarkingName = strPileDia + strCoverCode + strNoMainBar + lblMainBarDia + hdnPopUpLinkDia + strLinkPitch + "-" + strCageLength + txtEditStructlastthreedigits;
      }
      item.vchStructureMarkingName = strStructureMarkingName;
    }
    ...
    // Adding Cage dia
    let intCageDia: number = 0;
    intCageDia = parseInt(txtEditPileDia, 10) - 2 * parseInt(txtEditCover, 10);

    // Checking for config XML when user did not click the popup
    if (!sessionStorage.getItem("HoleConfigurationXML")) {
      try {
        // CreateConfigurationXML();
      } catch (ex) {
        sessionStorage.removeItem("HoleConfigurationXML");
    ...
    // Adding no of weld points
    let intNoOfWeldPoints: number = 0;
    let intSmallerMainBarLength: number = parseInt(item.intSmallerMainBarLength);
    let refMainBarList: any[] = [];
    let refSpiralLinkList: any[] = [];
    let refStiffnerList: any[] = [];
    let refCircularList: any[] = [];
    ...
    try {
      intNoOfWeldPoints = this.updateCalculateNoOfWeldPoints(
        parseInt(txtEditNoMainBar, 10),
        parseInt(txtEditNoLinksStart, 10),
        parseInt(txtEditNoLinksEnd, 10),
        parseInt(txtEditNoAdditionalSpiral, 10),
        parseInt(txtEditNoStiffnerRing, 10),
        parseInt(txtEditNoSquareStiffner, 10),
        intSmallerMainBarLength,
        refMainBarList,
        refSpiralLinkList,
        refStiffnerList,
        refCircularList,
        refCentralizerList,
        item.vchTactonConfigurationState
      );
    } catch (ex) {
      let errorLogNo = "";
      // errorLogNo = ErrorHandler.Publish(ex, window.location.href);
      // strErrorMsg = "alert('Please contact NDS Administrator with Error Log Ref Number:" + errorLogNo.toString() + "' );";
      // alert(strErrorMsg);
      alert("Please contact ODDS Administrator with Error Log Ref Number:" + errorLogNo.toString() + "' ");
    }

    // TypeScript equivalent code
    let strMachine: string = "";
    // strMachine = ddlMachineType.options[ddlMachineType.selectedIndex].text;
    // if (strMachine !== "") {
    //     if (strMachine.toLowerCase().includes("schnell")) {
    //         if (hdnAlternateSnlTemplates === "") {
    //             objDetailInfo.vchShenellAlternateTemplates = lblAlternateSnlTemplates.textContent !== "" ? lblAlternateSnlTemplates.textContent : "0";
    //         } else {
    //             objDetailInfo.vchShenellAlternateTemplates = hdnAlternateSnlTemplates;
    //         }

    //         if (hdnSnlTempId === "") {
    //             objDetailInfo.intShenellTemplateId = lblSnlTemplateId.textContent !== "" ? parseInt(lblSnlTemplateId.textContent, 10) : 0;
    //         } else {
    //             objDetailInfo.intShenellTemplateId = parseInt(hdnSnlTempId, 10);
    //         }
    //     } else {
    //         objDetailInfo.vchShenellAlternateTemplates = "0";
    //         objDetailInfo.intShenellTemplateId = 0;
    //     }
    // }


    // if (hdnChangeIndicator !== "") {
    //     if (hdnChangeIndicator === "2") {
    //         item.bitCageNoteChangeIndicator = lblChangeIndicator.textContent;
    //     } else {
    //         item.bitCageNoteChangeIndicator = hdnChangeIndicator;
    //     }
    // } else {
    //     item.bitCageNoteChangeIndicator = lblChangeIndicator.textContent;
    let txtStructRevNo: any = item.tntStructureRevNo;
    let lblGrpVer;
    let ddlParameter;

    // item.tntCageNoteId = parseInt(ddlCageNotes, 10);
    // item.intStructureMarkId = parseInt(lblStructId.textContent, 10);
    item.tntStructureRevNo = txtStructRevNo !== "" ? parseInt(txtStructRevNo, 10) : 0;

    let intgroupId: number = this.IntGroupMarkId ?? 0;
    // item.intGroupMarkId = intgroupId;
    // item.tntGroupRevNo = parseInt(lblGrpVer, 10);
    // item.tntParamSetNumber = parseInt(ddlParameter, 10);
    item.intCageSeqNo = parseInt(txtEditSeqNo, 10);
    item.intNoofWeldPoints = intNoOfWeldPoints;
    item.numPileDia = parseInt(txtEditPileDia, 10);
    item.numCageLength = parseInt(txtCageLength, 10);
    item.intCageDia = intCageDia;
    item.tntNumOfMainBar = parseInt(txtEditNoMainBar, 10);
    item.tntNumOfLinksatStart = parseInt(txtEditNoLinksStart, 10);
    item.tntNumOfLinksatEnd = parseInt(txtEditNoLinksEnd, 10);
    item.tntAdditionalSpiral = parseInt(txtEditNoAdditionalSpiral, 10);
    item.tntNumberOfStiffnerOrCentralizer = parseInt(txtEditNoStiffnerRing, 10);
    item.tntNumOfSquareStiffner = parseInt(txtEditNoSquareStiffner, 10);
    item.tntNumOfCircularRings = parseInt(txtNosCircularRing, 10);
    item.intLapLength = parseInt(txtEditLapLength, 10);
    item.intEndLength = parseInt(txtEditEndLength, 10);
    item.intCoverToLink = parseInt(txtEditCover, 10);
    item.intAjustmentFactor = parseInt(txtEditAdjustFactor, 10);
    item.intMachineTypeId = parseInt(ddlMachineType, 10);
    item.intMemberQty = parseInt(txtEditMemQty, 10);
    let mainBarList = item.vchTactonConfigurationState.mainBarList;
    if(mainBarList.length > 1){
      item.intSmallerMainBarLength = mainBarList.reduce((prev:any, current:any) => {
        return (parseInt(prev.main_bar_part_length_field) < parseInt(current.main_bar_part_length_field)) ? prev.main_bar_part_length_field : current.main_bar_part_length_field;
      });
    ...
    item.chrDrawingVersion = "";
    item.vchDrawingRemarks = "";
    item.vchTactonConfigurationState = JSON.stringify(item.vchTactonConfigurationState);
    item.vchHoleConfigurationXML = "";
    item.vchTactonXmlResult = "";
    ...
    item.vchOutputDrawingRef = "";
    item.vchBPCPdfPath = "";
    item.bitCoat = chkCage.checked ? true : false;
    item.bitAssemblyIndicator = chkAssembly.checked ? true : false;
    item.tntStatusId = 1;
    item.intCreatedUID = 0;
    item.intUpdatedUID = 0;
    item.vchLinkPitch = item.vchLinkPitch.toString()
    // added by debarati for CHG0032240
    // item.sitProductTypeId = parseInt(Session["productL2id"] || "0", 10);
    ...
    strSAPCodeID = await this.getBPCMaterialCodeIDForStrucMarking_Wrapper(item.vchMainBarPattern, item.vchElevationPattern, intSmallerMainBarLength, item.numPileDia, item.bitCoat, this.ParameterValues.sitProductTypel2Id);
    if (!isNaN(parseInt(strSAPCodeID))) {
      item.intBPCMaterialCodeID = parseInt(strSAPCodeID, 10);

      // Additional code for retrieving weight values from dataset
      if (!isNaN(parseFloat(strSAPCodeID))) {
        item.intBPCMaterialCodeID = strSAPCodeID;

        let ds: any;
        let INVWEIGHT = 0; //temp
        let PRODWEIGHT = 0;  //temp
        ds = await this.GetCABDetails(this.MeshData.INTSEDETAILINGID, 0);
        if (ds.length) {
          ds.forEach((element: any) => {
            INVWEIGHT += element.INVWEIGHT
            PRODWEIGHT += element.PRODWEIGHT
          });
          item.numStructActualWeight = PRODWEIGHT;
          item.numNetWeight = INVWEIGHT;
        } else {
          item.numStructActualWeight = 0;
          item.numNetWeight = 0;
      ...
      this.UpdateBPCStructureMarkingDetails(item);


    }
  }
  updateCalculateNoOfWeldPoints(
    mainBarCount: any,
    linksStart: any,
    linksEnd: any,
    addSpiralCount: any,
    ...
    let addSpiralList = dropdowns.additionalSpiralList;
    mainBarList = dropdowns.mainBarList;
    spiralLinkList = dropdowns.elevationList;
  ...
  EditCancel() {
    ...
    this.tabledata = JSON.parse(JSON.stringify(this.tabledata_back));

  }
  DeleteBPCStructureMarking(StructureMarkId: any, CageSeqNo: any, GroupMarkId: any) {
    this.BPCService.Delete_StructureMarking(StructureMarkId, CageSeqNo, GroupMarkId).subscribe({
      next: (response) => {
  ...
  deletebpcstructuremarking(item: any) {
    this.BPCService.Delete_StructureMarking(item.intStructureMarkId, item.tntCageSeqNo, item.intGroupMarkId)
      .subscribe({
        next: (response) => {
          console.log(response)
          if (response.response == "success") {
            ...
            this.tosterService.success("Failed...");
          }
        },
        error: (e) => {
          this.tosterService.error("Please check the Internet connection..", e.error);
          // alert("Cannot reload Beam Stirrup Cage detail infomation. Please check the Internet connection.");
        },
        complete: () => {
          this.getgriddata(this.MeshData.VCHGROUPMARKINGNAME, this.ProjectId, this.seDetailingID);
  ...
  machineTpeChanged(event: any, item: any) {
    let abvr = event.vchAbbr;
    console.log("machineTpeChanged=>", event);
    const modalRef = this.modalService.open(MachineTypeComponent, {
      size: 'lg', // 'lg' stands for large, adjust as needed
      centered: true, // Optional: Center the modal
    });
    modalRef.componentInstance.Insert_BPC_Structuremarking = item;
    modalRef.componentInstance.ParameterValues = this.ParameterValues;
    modalRef.componentInstance.commonPopupModel = this.commonPopupModel;
    modalRef.result.then(
      (result: any) => {
  ...
  showreport() {
    let BpcReport = "http://nsprddb10:8080/ReportServer/Pages/ReportViewer.aspx?%2fODOS_Reports%2frptBPCProductDetails&rs:Command=Render";
    let report = BpcReport + "&INTCUSTOMERID=" + 0 + "&INTCONTRACTID=" + 0 + "&INTPROJECTID=" + 0 + "&SITPRODUCTTYPEID=" + this.ProductTypeID + "&INTGROUPMARKID=" + this.IntGroupMarkId + "&rc:Parameters=false";
    window.open(report, '_blank');
  }

  reloadSBDetails() {
    // '0001101688', '0000112837'
    var lCustomerCode = this.CustomerCode; //'0001101688';//this.CustomerCode;
    var lProjectCode = this.ProjectCode; //'0000112837';
  ...
  GetCouplerType() {
    ...
    // ONLY USE IF THE COUPLERLIST IS NOT SET TO BE HARD-CODED
    // CHECK THE VALUE OF CUSTOEMR/PROJECT CODE ARE CORRECT
  ...
  selectRow(item:any,event:MouseEvent)
  {
    ...
    let i  = this.tabledata.findIndex(x=>x.intStructureMarkId ===item.intStructureMarkId);
  ...
  deleteAllselected()
  {
    this.tabledata.forEach(element => {
      if(element.colorRow)
      {
        this.deletebpcstructuremarking(element)
