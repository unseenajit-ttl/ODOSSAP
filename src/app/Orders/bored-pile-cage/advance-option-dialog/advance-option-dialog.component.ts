...
export class AdvanceOptionDialogComponent {
  ...
  cr_ring_type = "default";
  cr_bundle_side = "top";
  cr_link_lapping:number = 51;
  sr_link_lapping:number = 10;
  top_link:any
  ...
  ngOnInit() {
    ...
    if(this.lBPCRec.cr_ring_type){
      this.cr_ring_type = this.lBPCRec.cr_ring_type;
    }
    if(this.lBPCRec.cr_bundle_side){
      this.cr_bundle_side = this.lBPCRec.cr_bundle_side;
    }
    if(this.lBPCRec.mainbar_position_2layer){
      this.mainbar_position_2layer = this.lBPCRec.mainbar_position_2layer;
    }

    this.lLapLen = parseInt(this.lBPCRec.lap_length);
    this.lEndLen = parseInt(this.lBPCRec.end_length);
    this.cr_link_lapping = parseInt(this.lBPCRec.cr_link_lapping) ?? 51;
    this.sr_link_lapping = parseInt(this.lBPCRec.sr_link_lapping) ?? 10;
    this.top_link = this.lBPCRec.top_link;
  ...
  applyData(){
    ...
    let obj = {
            ...
            pdf_remark:this.remarks,
            cr_ring_type:this.cr_ring_type,
            cr_bundle_side:this.cr_bundle_side,
            cr_link_lapping : this.cr_link_lapping,
            sr_link_lapping : this.sr_link_lapping,
            top_link : this.top_link,
            mainbar_position_2layer:this.mainbar_position_2layer,
            twopcs_stiffener: this.twopcs_stiffener
          }
  ...
  setDefaultValues(){
    this.remarks = this.lBPCRec.pdf_remark ? this.lBPCRec.pdf_remark : '';
    if(
      ...
      if(this.lBPCRec.twopcs_stiffener ){
        this.twopcs_stiffener  = this.lBPCRec.twopcs_stiffener
        // console.log("twopcs_stiffener:",this.twopcs_stiffener)
      }
  }
  get showMixedAndDouble(): boolean {
    return (
      (this.lBPCRec.main_bar_type === 'Mixed') ||
      this.lBPCRec.pile_type == 'Double-Layer'
    );
  }
  get showDouble(): boolean {
    return (
        this.lBPCRec.pile_type == 'Double-Layer'
    );
  }
  get showMixed(): boolean {
    return (
      this.lBPCRec.main_bar_type === 'Mixed' &&
      (this.lBPCRec.main_bar_arrange == 'Side-By-Side' || this.lBPCRec.main_bar_arrange == 'In-Out')
    );
  }
  get showMixedSideBySide(): boolean {
    return (
      (this.lBPCRec.main_bar_type === 'Mixed' && this.lBPCRec.main_bar_arrange == 'Side-By-Side') || this.lBPCRec.pile_type == 'Double-Layer'
    );
  }
  ...
  // removeTemplateKeyword(keyword: number) {
  //   this.templateKeywords = this.templateKeywords.filter((k: number) => k != keyword);
  // }
  ...
  //   // Add our keyword
  //   if (value) {
  //     this.templateKeywords.push(parseInt(value));
  //   }
