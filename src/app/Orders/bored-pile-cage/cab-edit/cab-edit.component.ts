...
import { BPCService } from 'src/app/Detailing/MeshDetailing/bpc.service';
...
export class BPCCabEditComponent {

  @ViewChild('myCanvas', { static: true })
  Canvas!: ElementRef<HTMLCanvasElement>;
  cabGrid!: AngularGridInstance;
  dataViewCAB: any;
  ...
  ngAfterViewInit() {
    let pBPC: any;
    this.tranferService.setBpc(true);
    this.tranferService.data$.subscribe((data) => {
      pBPC = data;
      if (pBPC) {
        this.customerCode = pBPC.CustomerCode;
        this.projectCode = pBPC.ProjectCode;
        this.JobID = pBPC.JobID;
        this.CageID = pBPC.CageID;
        this.reloadRebars(pBPC);
        this.reloadSBDetails();
        this.getShapeCodeList(this.customerCode,this.projectCode,"No-Coupler");
      }
    });
  }
  reloadRebars(pBPC:any) {
    ...
    if (pBPC) {
      this.orderService.getRebarData_bpc(pBPC).subscribe((response: any) => {
        ...
        if (response != null && response.length > 0) {
          for (var i = 0; i < response.length; i++) {
            templateData[i] = {
              ...
              BarSize: response[i].BarSize.toString(),
          ...
          this.cabData = templateData;
          // this.dataViewCAB.addItems(this.cabData);
          // this.dataViewCAB.refresh();
          // this.cabGrid.slickGrid.invalidateAllRows();
          this.cabGrid.slickGrid.render();
          this.cabGrid.resizerService.resizeGrid();
          debugger;
        } else {
          this.cabGrid.resizerService.resizeGrid();
        }
  ...
  async Insert_BarMark(pGridID: any, pRowID: any): Promise<any> {
    ...
    if (pGridID != null && pRowID != null) {
      ...
      if (lItem.BBSID > 0 && lItem.BarID > 0) {
        ...
        //this.ReloadSubMenu();
        let gridArray = this.cabGrid.slickGrid;
  ...
  async InsertItem() {
    ...
    if (this.cabGrid.slickGrid.getActiveCell() != null) {
      var lRowIndex = this.cabGrid.slickGrid.getActiveCell().row;
      if (lRowIndex < this.dataViewCAB.getLength()) {
        ...
        if (result == true) {
          ...
          this.cabGrid.slickGrid.invalidate();
          this.cabGrid.slickGrid.render();
          this.cabGrid.slickGrid.setSelectedRows([lRowIndex]);
          this.cabGrid.slickGrid.setActiveCell(lRowIndex, 0);
  ...
  async DeleteBarDetails() {
    var lRows = this.cabGrid.slickGrid.getSelectedRows();
    if (lRows.length > 0) {
      ...
      for (let i = lRows.length - 1; i >= 0; i--) {
        var lItem = this.cabGrid.slickGrid.getDataItem(lRows[i]);
        if (lItem != null) {
          var lSuccess = 1;
      ...
      this.cabGrid.slickGrid.invalidate();
      this.cabGrid.slickGrid.render();
      this.cabGrid.slickGrid.setSelectedRows([lRows[0]]);
      this.cabGrid.slickGrid.setActiveCell(lRows[0], 0);
      lData = null;
    }
  }
  ...
  async ClearRows() {
    let lRows = this.cabGrid.slickGrid.getSelectedRows();
    if (lRows.length < 1) {
    ...
    if (
      ...
      for (let i = 0; i < lRows.length; i++) {
        let lItem: SaveBarDetailsModelDetailing = this.cabGrid.slickGrid.getDataItem(lRows[i]);
        ...
        let id = this.cabGrid.slickGrid.getDataItem(lRows[i]).id;
  ...
  CopyRows() {
    ...
    let copyItem = [];
    for (let i = 0; i < lRows.length; i++) {
      let item = this.cabGrid.slickGrid.getDataItem(lRows[i]);
      copyItem.push(item);
    }
  ...
  PasteRows() {
    ...
    var lRowIndex = 0;
    if (this.cabGrid.slickGrid.getActiveCell() != null) {
      lRowIndex = this.cabGrid.slickGrid.getActiveCell().row;
    }
    var lCustomerCode = this.customerCode; // document.getElementById("CustomerCode").value;
    var lProjectCode = this.projectCode; // document.getElementById("ProjectCode").value;
    var lJobID = this.JobID; // document.getElementById("OrderNo").value;
  ...
  async grid_onActiveCellChanged(e: any, args: any) {
    ...
    let rowNum = this.cabGrid.slickGrid.getActiveCell().row;
    let lItem = this.cabGrid.slickGrid.getDataItem(rowNum);
    if (lItem == undefined) {
      return;
    }
    ...
    // Get the active cell
    const activeCell = this.cabGrid.slickGrid.getActiveCell();
  ...
  async grid_onCellChange(e: any, args: any) {
    ...
    if (lColumnName == 'BarShapeCode') {
      ...
      if (lShapeCode != null && lShapeCode != '') {
        ...
        if (lTotalLength != null && lTotalLength != '') {
          ...
          //   let lTotalDed: any = await this.getCreepDedution(lMaxLength, args.item);
    ...
    if (lColumnName == 'BarMemberQty' || lColumnName == 'BarTotalQty') {
      var lMQty = 1;// args.item['BarMemberQty'];
      var lEQty = args.item['BarTotalQty'];
      if (lColumnName == 'BarMemberQty' && !isNaN(lMQty) && lMQty > 1000) {
        alert('Are you sure Member Qty is ' + lMQty + '?');
      }
      if (lColumnName == 'BarTotalQty' && !isNaN(lEQty) && lEQty > 1000) {
    ...
    if(lColumnName == "Remarks"){
      ...
      //     );
      //     // return false;
      //     this.cabGrid.slickGrid.setActiveCell(args.row, args.cell); // Keep focus on the same cell
      //     // this.cabGrid.slickGrid.editActiveCell(args.row, args.cell);
      //   }
      // }
    }
  ...
  angularGridReady(event: Event) {
    ...
    this.cabGrid = (event as CustomEvent).detail as AngularGridInstance;
    this.dataViewCAB = this.cabGrid.slickGrid.getData();

    this.cabGrid.slickGrid.onValidationError.subscribe((e, args) => {
      alert(args.validationResults.msg);
    });

    this.cabGrid.slickGrid.onActiveCellChanged.subscribe((e, args) => {
      this.grid_onActiveCellChanged(e, args);
    });
    this.cabGrid.slickGrid.onBeforeEditCell.subscribe((e, args) => {
      this.grid_onBeforeEditCell(e, args);
    });
    this.cabGrid.slickGrid.onCellChange.subscribe((e, args) => {
      debugger;
      this.grid_onCellChange(e, args);
    });
    this.cabGrid.slickGrid.onAddNewRow.subscribe((e, args) => {
      this.grid_onAddNewRow(e, args);
    });
    this.cabGrid.slickGrid.onSelectedRowsChanged.subscribe((e, args) => {
      this.grid_onSelectedRowsChanged(e, args);
    });
    this.cabGrid.slickGrid.onClick.subscribe((e, args) => {
      this.grid_onClick(e, args);
    });
    this.cabGrid.slickGrid.onDblClick.subscribe((e, args) => {
      console.log('Open ShapeList Component');
    });

    this.cabGrid.slickGrid.onKeyDown.subscribe((e, args) => {
      this.grid_onKeyDown(e, args);
    });
    this.cabGrid.slickGrid.onContextMenu.subscribe((e, args) => {
      this.grid_onContextMenu(e, args);
    });
    this.cabGrid.slickGrid.onSort.subscribe((e, args) => {
      this.grid_onSort(e, args);
    });
  }

  setCellClass(row: number, column: number, cssClass: string) {
    const cell = this.cabGrid.slickGrid.getCellNode(row, column);
    if (cell) {
  ...
  InsertColumn(pPara: any, pGridID: any) {
    let Grid = this.cabGrid.slickGrid;
    let dataViewCAB;
    ...
    const allColumns = this.cabGrid.gridService.getAllColumnDefinitions();
    allColumns.map((data) => {
      if (data.id == 'A') {
    ...
    this.cabGrid.gridService.resetGrid(allColumns);
  ...
  async grid_onSelectedRowsChanged(e: any, args: any) {
    let dataViewArray = this.dataViewCAB;
    let gridArray = this.cabGrid.slickGrid;
    let lItem = dataViewArray.getItem(args.previousSelectedRows[0]);
    this.getShapeFinderReset(this.gridIndex);
    if (args.rows.length > 0) {
      if (args.rows[0] != this.barRowIndex[this.gridIndex]) {
          ...
          // validate the Total Length
          lItem['BarLength'] = lTotalLength;
          var lMaxLength = this.getVarMaxValue(lTotalLength);
          var lBarSize = lItem['BarSize'];
          ...
          // let lTotalDed: any = await this.getCreepDedution(lMaxLength, lItem);
          // if(lMaxLength - lTotalDed > lLenLimit) {
          //     alert(
          //       'Total bar cut length is ' +
          //       (lMaxLength - lTotalDed) +
          ...
          //     );
          //     // return false;
          //     gridArray.setActiveCell(args.previousSelectedRows[0], 15); // Keep focus on the same cell
          //     this.displayShapeImage(this.gridIndex, args.previousSelectedRows[0]);
          //     // this.cabGrid.slickGrid.editActiveCell(args.row, args.cell);
          ...
          //   );
          //   gridArray.setActiveCell(args.rows, 1);
          // }
          this.SaveBarDetails(
            this.gridIndex,
            this.barRowIndex[this.gridIndex]
        ...
        if (lData != null) {
          ...
          //   lMinLength < 800
          // ) {
          //   let lTotalDed: any = await this.creepDeduction(lMinLength, lData); //await this.getCreepDedution(lMinLength, lData);
  ...
  async getShapeInfo(
    ...
    try {
      const data = await this.orderService
        .getShapeInfo(this.customerCode, this.projectCode, this.JobID, ShapeCode)
        .toPromise();
      return data;
    } catch (error) {
      console.error(error);
  ...
  metadata(old_metadata: any) {
    return (row: any) => {
      let item: any = this.cabGrid.slickGrid.getDataItem(row);
  ...
  grid_onKeyDown(e: any, args: any) {
    ...
    if (e.which == 9) {
      ...
      if (lColumnName == 'BarShapeCode') {
        ...
        if (
          ...
          let lItem = this.cabGrid.slickGrid.getDataItem(lCurrRow);
          lItem.BarShapeCode = '';
          this.dataViewCAB.updateItem(lItem.id, lItem);
    ...
    if (e.which == 13) {
      ...
      if (args.grid.getDataItem(lCurrRow) != null) {
        ...
        if (lColumnName == 'BarTotalQty') {
          args.grid.getEditorLock().commitCurrentEdit();
          var lEachQty = args.grid.getDataItem(lCurrRow).BarTotalQty;
          if (
            lEachQty == null ||
            lEachQty == undefined ||
            lEachQty == '' ||
                ...
                if (lMbrQty > 1) {
                  ...
                  if (lHeight < 18) {
                    ...
                    if (lCheckLen == true) {
                      ...
                      var lSuEqty = pItem['BarTotalQty'] * 2;
                      if (
                        lMbrQty == lSuMQty * 2 &&
        ...
        for (var i = 0; i < lFormula1AR.length; i++) {
          ...
          if (lFormula1.indexOf(pColumnName) >= 0) {
            ...
            if (lResult != '') {
              ...
              //clear the highlighted error
              var lClass =
                this.cabGrid.slickGrid.getCellCssStyles('error_highlight');
              if (lClass == undefined) {
                lClass = {};
              }
              if (lClass != null) {
                if (lClass[this.dataViewCAB.getRowById(pItem.id)] != null) {
                  ...
                  this.cabGrid.slickGrid.setCellCssStyles(
                    'error_highlight',
                    lClass
                  );
                }
              ...
              for (var j = 0; j < lFormula1AR.length; j++) {
                ...
                if (lFormula1.indexOf(lParam) >= 0 && lParam1 != pColumnName) {
                  ...
                  if (lResult != '') {
                    ...
                    //clear the highlighted error
                    var lClass =
                      this.cabGrid.slickGrid.getCellCssStyles(
                        'error_highlight'
                      );
                    if (lClass == undefined) {
                      lClass = {};
  ...
  parameterValidator(args: any, value: any, columnName: any) {
    ...
    if (isNaN(lValue) == true) {
      if (lValue.indexOf('-') <= 0) {
        ...
        //Check Various Bar
        var lEachQty = lItem.BarTotalQty;
      ...
      if (response.length > 0) {
        for (let i = 0; i < response.length; i++) {
          ...
          if (response[i].BarSTD == true) {
            ...
            if (lPcsFr > 0) {
              if (lPcsFr != lPCs) {
                data[i].BarTotalQty = lPcsFr + '-' + lPCs;
                data[i].BarTotalQty =
  ...
  CheckBBSDetails() {
    ...
    // this.cabGrid.slickGrid.getEditorLock().commitCurrentEdit();
    // SaveBarDetails(i, barRowIndex[i]);
    this.cabGrid.slickGrid.removeCellCssStyles('error_highlight');
    let lClass: any = {};
    var lCellClass = '';
    for (let j = 0; j < this.dataViewCAB.getLength(); j++) {
      ...
      if (this.dataViewCAB.getItem(j).Cancelled != true) {
        ...
        if (
          lShape != null &&
          lShape.trim() != '' &&
          (this.dataViewCAB.getItem(j).BarTotalQty == null ||
            this.dataViewCAB.getItem(j).BarTotalQty == '' ||
            this.dataViewCAB.getItem(j).BarTotalQty == '0' ||
            this.dataViewCAB.getItem(j).BarTotalQty == ' ')
        ) {
          lClass[j]['BarTotalQty'] = 'highlighted';
        }
          ...
          for (let k = 0; k < lParaA.length; k++) {
            ...
            if (
              ...
              if (lArrayP.length == 2) {
                ...
                var lEachQty = this.dataViewCAB.getItem(j)['BarTotalQty'];
    ...
    this.cabGrid.slickGrid.setCellCssStyles('error_highlight', lClass);
  }
    ...
    this.orderService
      ...
      .subscribe({
        next: (response) => {
          ...
          for (var i = 0; i < response.length; i++) {
            this.gShapeCodeList =
              this.gShapeCodeList + ',' + response[i];
  ...
  EditCancel() {
    ...
    this.GetTableData = JSON.parse(JSON.stringify(this.tabledata_back));
  ...
  giveRowcolor(item: any) {
   ...
   goBack()
  {
    let lRowNo = this.cabGrid.slickGrid.getSelectedRows()[0]
    if(lRowNo>=0){
      this.SaveBarDetails(lRowNo,lRowNo);
    }
    ...
    if (pGridID != null && pRowID != null) {
      ...
      if (lItem.BarID > 0) {
        ...
        let obj: any = {
          ...
          BarShapeCode: lItem.BarShapeCode ? lItem.BarShapeCode.toString() : '',
          A: lItem.A ? Number(lItem.A) : 0,
          B: lItem.B ? Number(lItem.B) : 0,
          C: lItem.C ? Number(lItem.C) : 0,
          D: lItem.D ? Number(lItem.D) : 0,
          E: lItem.E ? Number(lItem.E) : 0,
          F: lItem.F ? Number(lItem.F) : 0,
          G: lItem.G ? Number(lItem.G) : 0,
          BarLength: Number(lItem.BarLength),
          BarWeight: Number(lItem.BarWeight),
          Remarks: lItem.Remarks ? lItem.Remarks.toString() : '',
        };
        this.updateCabDetails(obj);
        return lReturn;
      }
    }
  }
  updateCabDetails(resp:any){
    this.orderService.updateBPCCabDetails(resp).subscribe((response)=>{
