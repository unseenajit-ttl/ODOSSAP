...
@Component({
  selector: 'app-ctsmeshorder',
  templateUrl: './ctsmeshorder.component.html',
  styleUrls: ['./ctsmeshorder.component.css']
})
export class CtsmeshorderComponent {
  ProductType: any;
  ScheduleProd: any;
 ...
 async ngOnInit() {  
  ...
  showshapelistimages()
  {
    ...
    //     });
    //   });
    this.orderService.getShapeImagesByCat_beam("OTHERS").subscribe((imageData: any) => {
      //debugger;
 ...
 loadMainFile(Imagename: any) {
  ...
  getRowData(item: any = [], index:any)
  {
    ...
    this.ProdMWDia=item.ProdMWDia;
    this.ProdMWSpacing=item.ProdMWSpacing;
    this.ProdCWDia=item.ProdCWDia;
    this.ProdCWSpacing=item.ProdCWSpacing;
    this.Mass=item.ProdMass;
    this.MeshTotalWT=item.MeshTotalWT;
