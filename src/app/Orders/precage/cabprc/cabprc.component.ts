...
import { BeamMeshArray, ColumnMeshArray, cabprcArray } from 'src/app/Model/StandardbarOrderArray';
...
@Component({
  selector: 'app-cabprc',
  templateUrl: './cabprc.component.html',
  styleUrls: ['./cabprc.component.css']
})
export class CabprcComponent {
  ...
  bbsOrderTable: cabprcArray[] = [];
  bbsOrderTemp:cabprcArray[] = [];
  shapeCodeList: any[] = [];
  sizeList: any[] = [];
  Diameter: any;
  Spacing: any;
  ...
  ngOnInit(): void {  
    //console.log('cab prcSharedS',this.prcSharedS)
    this.MemberQty= this.prcSharedS.MemberQty;
    ...
    // this.GetTableData(this.BBSID, false, 0,this.BBSID);
  ...
  getJobId(orderNumber: string): any {
    ...
    this.orderService.getJobId(orderNumber,ProdType,StructurEelement,ScheduleProd).subscribe({
      ...
      error: () => { },
      complete: () => {
        this.GetTableData(this.BBSID, false, 0,this.MemberQty);
  ...
  ShowProductList() {
    ...
    this.orderService.getProductList_beam(lCategory).subscribe({
      ...
      complete: () => {
        ...
        modalRef.componentInstance.StructureElement = "CAB";
 ...
 loadMainFile(Imagename: any) {
  ...
  getRowData(item: any = [], index:any)
  {
    ...
    console.log("shapecodestesting=?>", item);
    ...
    this.loadMainFile(item.BarShapeCode);
  ...
  lCoupler: any;
  GetTableData(
    pBBSID: any, pByNSH: any, pGroupMarkID:any, pMemberQty:any
  ) {
  ...
  this.orderService
      .getBarDetails_prc(this.CustomerCode, this.ProjectCode, this.JobID, this.BBSID,pByNSH,pGroupMarkID,pMemberQty)
      .subscribe({
        next: (response) => {
          ...
          for (var i = 0; i < response.length; i++) {
            var lShape = response[i].BarShapeCode;
            if (lShape != null && lShape.length > 0) {
                lShape = lShape.trim();
                var lType = lShape.substring(0, 1);
                if (lType == "H" || lType == "I" || lType == "J" || lType == "K") {
                    this.lCoupler = "N-Splice";
                }
                else if (lType == "C" || lType == "P" || lType == "N" || lType == "S") {
                  this.lCoupler = "E-Splice(S)";
                }
            }
        ...
        error: (e) => {
          this.toastr.error("Cannot reload bar detail infomation. Please try again later.."); 
    ...
    //   {
    //   this.orderService
    //   .GetBarDetails(CustomerCode, ProjectCode, PostID, this.BBSID)
    //   .subscribe({
    //     next: (response) => {
    //       //console.log('BBSORDERDETAILS', response);
