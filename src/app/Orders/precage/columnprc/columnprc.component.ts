...
@Component({
  selector: 'app-columnprc',
  templateUrl: './columnprc.component.html',
  styleUrls: ['./columnprc.component.css']
})
export class ColumnprcComponent {
  ProductType:any;
 ...
 loadMainFile(Imagename: any) {
  ...
  ShowProductList() {
    ...
    this.orderService.getProductList_beam(lCategory).subscribe({
      next: (response) => {
        let productList;
        //console.log('getProductList_Column', response);
    ...
    if(this.ScheduledProd=="Y")
    {
      ...
      .subscribe({
        next: (response) => {
          ...
          console.log("columnbbsOrderTable=>",this.bbsOrderTable);
          this.TotalWeight=0;
