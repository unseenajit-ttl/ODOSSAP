...
@Component({
  selector: 'app-documents-attached',
  templateUrl: './documents-attached.component.html',
  styleUrls: ['./documents-attached.component.css'],
})
export class DocumentsAttachedComponent implements OnInit {
  ...
  getAttachedTableList() {
    let OrderNumber = this.selectedRow.JobID; //1367; //this.dropdown.getCustomerCode(); //'0001101170';//this.selectedRow[0].CustomerCode;
    ...
    let ProductType = this.selectedRow.ProdType; //'CAB'; //this.JobID; //735 ;//this.selectedRow[0].JobID;
  ...
  UpdateDateFormat(date: any) {
    if (date) {
      // let date2 = moment(new Date(date)).format('YYYY-MM-DD HH:mm:ss')
      // console.log('date2', date2);

      // Use US format for all locale strings to prevent discrepancies between browsers.
