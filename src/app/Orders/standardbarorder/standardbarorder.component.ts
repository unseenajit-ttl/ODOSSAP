...
@Component({
  selector: 'app-standardbarorder',
  templateUrl: './standardbarorder.component.html',
  styleUrls: ['./standardbarorder.component.css'],
})
export class StandardbarorderComponent {
  receivedData: any = '';
  ...
  download() {
    ...
    // this.orderService.showdir().subscribe((data) => {
    //   const blob = new Blob([data], { type: 'application/pdf' });
    //   const url = window.URL.createObjectURL(blob);
    //   const a = document.createElement('a');
    //   a.href = url;
    ...
    // });
    // // debugger;
    // //  const pdfUrl = "D:/ODOS/StandardBarPDF/STDMESH-"+ this.ProjectCode + "-" + this.JobID + ".pdf";
  ...
  calculateTotalBundleWt() {
    ...
    const decimalPlaces = 3; // Change this to the desired number of decimal places
...
greentType="Y";
    ...
    this.orderService.getStandardProducts(prodtype,greentType).subscribe({
      next: (response) => {
        // Filter the response array to get the products that match the expected product type.
        ...
        this.StandardbarOrderArray_backup = JSON.parse(
          JSON.stringify(this.StandardbarOrderArray)
        );
        //allowGrade500M
    ...
    this.orderService
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.JobID = response.StdBarsJobID;
      ...
      .subscribe({
        ...
        complete: () => {
          ...
          if (
            this.OrderStatus == 'Created' ||
            this.OrderStatus == 'Created*' ||
            (this.OrderStatus == 'Sent' && this.Submission == 'Yes')
          ) {
  ...
  filterStandardbarOrderArray() {
    this.StandardbarOrderArray = JSON.parse(
      JSON.stringify(this.StandardbarOrderArray_backup)
    );

    if (this.searchProductCode) {
      this.UpdateDataList(this.searchProductCode, 'ProdCode');
    }
    if (this.searchProductDesc) {
      this.UpdateDataList(this.searchProductDesc, 'ProdDesc');
    }
    if (this.searchDiameter) {
      this.UpdateDataList(this.searchDiameter, 'Diameter');
    }
    if (this.searchGrade) {
      this.UpdateDataList(this.searchGrade, 'Grade');
    }
    if (this.searchUnitWeight) {
      this.UpdateDataList(this.searchUnitWeight, 'UnitWT');
    }
  }

  UpdateDataList(pValue: string, pColumnName: string) {
    if (
      pColumnName == 'ProdCode' ||
      pColumnName == 'ProdDesc' ||
      pColumnName == 'Diameter' ||
      pColumnName == 'Grade' ||
      pColumnName == 'UnitWT'
    ) {
      this.StandardbarOrderArray = this.StandardbarOrderArray.filter((item) =>
        item[pColumnName]
          .toString()
          ?.toLowerCase()
          .includes(pValue?.toLowerCase())
      );
    }
  }
  ...
  reloadProjectDetails(CustomerCode: any, ProjectCode: any) {
    this.orderService
      .reload_ProjectDetails_Std(CustomerCode, ProjectCode)
      .subscribe({
        next: (response) => {
  ...
  //     error: (e) => {},
  //     complete: () => {
  //       if(this.StandardbarOrderArray?.length>0){
  //       this.UpdateProductList(this.gGreenSteelSelection);
  //       }
  //     },
  //   });
