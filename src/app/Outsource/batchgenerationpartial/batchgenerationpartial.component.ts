...
@Component({
  selector: 'app-batchgenerationpartial',
  templateUrl: './batchgenerationpartial.component.html',
  styleUrls: ['./batchgenerationpartial.component.css']
})
export class BatchgenerationpartialComponent implements OnInit {
 ...
 isLoading = false;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<BatchgenerationpartialComponent>,
  ...
  ngOnInit(): void {
    ...
    const getFullOrders = localStorage.getItem('outsourceorders');
    this.OutsourceType = localStorage.getItem('OutsourceType');
    ...
    // this.orderService.getSupplierData(productType).subscribe({
    //   next: (response) => {
    //     if (response.success) {
    //       this.supplierList = response.data;
    //       console.log("SuppliersList=>",this.supplierList)
    //     } else {
  ...
  onServiceChange(value: string) {
    switch (value) {
      case 'CAB':
        this.processOptions = ['Bending', 'Coupling'];
        break;
      case 'MESH':
        this.processOptions = ['Bending'];
        break;
      case 'BPC':
        this.processOptions = ['Secondary', 'Fabrication'];
        break;
      case 'PRC':
        this.processOptions = ['Fabrication'];
        break;
      default:
        this.processOptions = [];
    }
  ...
  getVendorList() {
    this.orderService.getVendorList(
  ...
  assignPartialOutSource() {
    if (
      ...
      // this.selectedService === "" || this.selectedService === undefined || this.selectedService === null||
      //   this.selectedProcess === "" || this.selectedProcess === undefined || this.selectedProcess === null
    ...
    if (!supplier) {
      this.toastr.error("Selected supplier not found in the list!");
      return;
    }
    const soNums = this.FullOutsourceOrders.map(o => o.order_no);
    this.orderService.assignOrderOutSource(
      soNums,
      this.OutsourceType,
      ...
      next: (response) => {
        console.log('Data saved successfully:', response);
        alert("Assigning completed for all Partial Outsourced orders.")
      },
      error: (error) => {
        console.error('Error saving data:', error);
      },
