...
import { EmailNotificationComponent } from '../../../Orders/upcoming-orders/email-notification/email-notification.component';
...
@Component({
  selector: 'app-attachdocuments',
  templateUrl: './attachdocuments.component.html',
  styleUrls: ['./attachdocuments.component.css'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class AttachDocumentComponent {
  ...
  async ngOnInit() {
    ...
    this.TntStatusId = this.SelectedRecord.TNTSTATUSID;
...
//   }
  ...
  async ModifyItem(DrawingID: any, Revision: any, Remarks: any) {
    try {
      let obj = {
        DrawingID: DrawingID,
        Revision: Revision,
  ...
  async Modify_button() {
    ...
    let Revision ;; //this.ProjectCode; //this.selectedRow[0].ProjectCode;
    ...
    for (let i = 0; i < this.ListofDocuments.length; i++) {
      ...
      if (this.ListofDocuments[i].isSelected == true) {
        ...
        Revision = this.ListofDocuments[i].Revision;
        if (DrawingId == 0) {
          alert(
        ...
        this.DocumentLoading = true;
        let response = await this.ModifyItem(DrawingId, Revision, Remarks);
      }
    }
  ...
  async Upload() {
    ...
    for (let i = 0; i < lPdfFile.files.length; i++) {
      ...
      {
        await this.uploadFileasync(lPdfFile.files[i],this.selectedArray[j]);
  ...
  async uploadFileasync(file: any,item:any): Promise<any> {
    try {
      let IsDuplicate = await this.GetFileSubmissionStatus_new(this.CustomerCode,this.ProjectCode,file.name,item.INTWBSELEMENTID);
      IsDuplicate = IsDuplicate.result?1:0;
      let customer = this.CustomerCode;
      let project = this.ProjectCode;
      let OrderNumber = item.INTWBSELEMENTID;
      let DrawingNo = this.DocumentNo;
      let Remarks = this.Remarks;
      let WBS1 = item.VCHWBS1;
      let WBS2 = item.VCHWBS2;
      let WBS3 = item.VCHWBS3;
      let ProdType = item.ProductType;
      let StructureElement = item.StructureElement;
      let UploadType = 'R';
      let ScheduledProd = this.ScheduledProd;
      let Revision = 0;
      const data = await this.wbsService
  ...
  getTableHTML(): string {
    ...
    <span><strong>DRAWING SUBMITTED FOR APPROVAL  (图纸已提交审批)</strong></span>
      ...
      <table style="width:100%; margin-top: 20px;" cellpadding="0" border="1" class="x_MsoNormalTable">
        <thead>
          <tr>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Block (WBS1) <br>(<span lang="ZH-CN">座号</span>/<span lang="ZH-CN">大牌</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Storey (WBS2) <br>(<span lang="ZH-CN">楼层</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Part (WBS3) <br>(<span lang="ZH-CN">分部</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Structure Element <br> (构件)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> BBS Number <br> (BBS加工表号) </th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Product Type <br> (产品类型)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> PO Number <br> (订单号码) </th>
            <th style="width:10%; padding:.75pt .75pt .75pt .75pt"> Required Date <br> (到场日期) </th>
            <th style="width:20%; padding:.75pt .75pt .75pt .75pt"> BBS Description (BBS说明) </th>
          </tr>
        </thead>
        <tbody>
          ${this.selectedArray.map((selectedElement: any) => `
            <tr>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.VCHWBS1}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.VCHWBS2}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.VCHWBS3}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.StructureElement}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.VCHBBSNO}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.ProductType}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal"> </p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.ReqDate}</p></td>
              <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${selectedElement.BBS_DESC}</p></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    <table style="width:100%; margin-top: 20px;" cellpadding="0" border="1" class="x_MsoNormalTable">
      ...
      <tbody>
        ${this.ListofDocuments.map((drawing: any) => `
          <tr>
            <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${drawing.FileName}</p></td>
            <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${drawing.Revision}</p></td>
            <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${drawing.Remarks}</p></td>
            <td style="padding:.75pt .75pt .75pt 10pt"><p class="x_MsoNormal">${drawing.UpdatedDate}</p></td>
          </tr>
        `).join('')}
      </tbody>
    </table>
  ...
  getTableHTML_AfterRejection(): string {
    ...
    <span><strong>DRAWING SUBMITTED FOR APPROVAL  (图纸已提交审批)</strong></span>
      ...
      <table style="width:100%; margin-top: 20px;" cellpadding="0" border="1" class="x_MsoNormalTable">
        <thead>
          <tr>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Block (WBS1) <br>(<span lang="ZH-CN">座号</span>/<span lang="ZH-CN">大牌</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Storey (WBS2) <br>(<span lang="ZH-CN">楼层</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Part (WBS3) <br>(<span lang="ZH-CN">分部</span>)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Structure Element <br> (构件)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> BBS Number <br> (BBS加工表号) </th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Product Type <br> (产品类型)</th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> PO Number <br> (订单号码) </th>
            <th style="width:10%; padding:.75pt .75pt .75pt 10pt"> Required Date <br> (到场日期) </th>
            <th style="width:20%; padding:.75pt .75pt .75pt 10pt"> BBS Description (BBS说明) </th>
          </tr>
        </thead>
        <tbody>
          ${this.selectedArray.map((selectedElement: any) => `
    ...
    <table style="width:100%; margin-top: 20px;" cellpadding="0" border="1" class="x_MsoNormalTable">
      <thead>
        <tr>
          ...
          <th style="width:25%; padding:.75pt .75pt .75pt .75pt">Customer Remark</th>
  ...
  next:(response)=>{
    if(response)
    {
      ...
      if(this.user_list_new.length)
      {
        ...
        if(this.TntStatusId==3 || this.TntStatusId==12)
        {
          this.IsApproved_disabled  = true;
...
toggle_modify():void
{
  ...
  for (let element of this.ListofDocuments) {
...
async GetFileSubmissionStatus_new(customercode:string, projectcode:string,filename:string,wbselementid:number)
{
  try {
        let data = await this.wbsService.GetFileSubmissionStatus(customercode,projectcode,filename,wbselementid).toPromise();
        return data;
  } catch (error) {
    console.error(error);
    return false;
