...
import { AttachDocumentComponent } from './attachdocuments/attachdocuments.component';
import { WbsDocumentsAttachedComponent } from './documents-attached-posted/documents-attached-posted.component';
import { PrecastComponent } from 'src/app/Orders/precast/precast.component';
import { PrecastpopupComponent } from 'src/app/Orders/precastpopup/precastpopup.component';
import { GroupprecastpopupComponent } from 'src/app/Orders/groupprecastpopup/groupprecastpopup.component';
...
export class wbspostingComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    this.selectedcustomerName = this.dropdown.getCustomerCode();
    this.SelectedProjectID = this.dropdown.getDetailingProjectId();
    // this.selectedprojectName = '0000113012';
    this.selectedprojectName = this.dropdown.getProjectCode();
    //0000113012
    ...
    // alert("this.SelectedProjectID",this.SelectedProjectID);
...
open(item: any) {
  ...
  const obj = {
    ProjectCode: localStorage.getItem("DetailingProjectCode"),
    CustomerCode: this.selectedcustomerName,
    IsPrecast: item.IsPreCast,
    PostHeaderID: item.INTPOSTHEADERID,
    statusCode: item.CHRSTATUSCODE
  };
  // localStorage.setItem('PrecastDetail', JSON.stringify(obj));

  // if item is precast → open GroupprecastpopupComponent
  if (item.IsPreCast) {
    ...
    const modalRef = this.modalService.open(GroupprecastpopupComponent, ngbModalOptions);
    ...
    // if not precast → open AddGroupMarkComponent
    const ngbModalOptions: NgbModalOptions = {
      backdrop: 'static',
      keyboard: false,
      size: 'lg',
    ...
    modalRef.componentInstance.saveTrigger.subscribe((x: any) => {
      if (x) {
        this.wbspostingarray.forEach((element) => {
          element.isSelected = item.INTWBSELEMENTID === element.INTWBSELEMENTID;
        });
...
//     backdrop: 'static',
//     keyboard: false,
//     size: 'xxl',   // wide modal to fit both panels
//     centered: true,
//     container: 'body'
//   };
 ...
 */
  ...
  ValidatePrecast(item: any) {
    // if(item.ProductType=="MSH"){
    //   console.log('below');
    //   // console.log(this.selectedprojectName);
    //   let project="0000113012";
    //   // alert("test");
    //   console.log("currentItem-", item);
    //   this.wbsService.GetPrecastCount(project, this.selectedcustomerName, item.INTPOSTHEADERID).subscribe({
    //     next: (response) => {
  ...
  // NavigateToPrecast(item: any) {

  //   const modalRef = this.modalService.open(PrecastpopupComponent, {
  ...
  //   modalRef.componentInstance.structure = item.StructureElementID;
  //   modalRef.componentInstance.postHeaderId = item.INTPOSTHEADERID;
  //   // console.log('checking now-', item);
  //   // alert(this.selectedprojectName[0]);
  //   let obj = {
  //     CustomerCode: this.selectedcustomerName,
  //     ProjectCode: localStorage.getItem("DetailingProjectCode"),
  ...
  //   localStorage.setItem('wbspostingobject', JSON.stringify(obj));
  //   // this.router.navigate(['/order/createorder/Precast']);
  // }
  openDialog() {
    if (this.selectedcustomerName && localStorage.getItem("DetailingProjectCode")) {
      const modalRef = this.modalService.open(AttachDocumentComponent, {
        size: 'xl', // 'lg' stands for large, adjust as needed
        centered: true, // Optional: Center the modal
        windowClass: 'your-custom-dialog-class'
      });
      modalRef.componentInstance.customerCode = this.selectedcustomerName;
      modalRef.componentInstance.projectCode = localStorage.getItem("DetailingProjectCode");
  ...
  openDocument(item: any) {
    ...
    if(!item.isSelected)
    {
      this.toastr.warning("Please select the WBS");
      return ;
    }
    for (let i = 0; i < this.wbspostingarray.length; i++) {
      if (this.wbspostingarray[i]['isSelected'] == true) {
      //  if(this.wbspostingarray[i].DrawingStatus!==null || this.wbspostingarray[i].DrawingStatus!=='Submited')
      //  {
      //   this.toastr.warning("Please select the WBS which have status as  Submitted or Not Not Submitted")
      //   return ;
      //  }

        selectedArray.push(this.wbspostingarray[i])
    ...
    modalRef.componentInstance.SelectedRecord = item;
    modalRef.componentInstance.selectedArray = selectedArray;
    modalRef.componentInstance.saveTrigger.subscribe((x: any) => {
  ...
  viewDocuments(item: any, pEvent: MouseEvent) {
  ...
  status(status: boolean, item: any) {
    ...
    this.wbsService.Drawing_Approval(item.INTWBSELEMENTID, status).subscribe({
      next: (Response) => {
        this.toastr.success("Status approved Successfully");
  ...
  drawingReport()
  {
    if(this.selectedCustomer && this.SelectedProjectID){
      this.router.navigate(['/wbs/wbsposting/drawingreport'],{
        queryParams: { module: 'wbs',CustomerCode: this.selectedCustomer,ProjectCode:this.SelectedProjectID},
      });
    }else{
